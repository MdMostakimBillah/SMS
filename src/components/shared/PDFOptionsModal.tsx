...
export const PDFOptionsModal = React.memo(function PDFOptionsModal({
  count, isBn, onClose, onDownload,
}: Props) {
  const [cols, setCols]         = useState<string[]>(ALL_PDF_COLUMNS.filter(c => c.default).map(c => c.key))
  const [title, setTitle]       = useState('Student List')
  ...
  const selectAll      = useCallback(() => setCols(ALL_PDF_COLUMNS.map(c => c.key)), [])
  ...
  const updateEmptyCol = useCallback((i: number, val: string) =>
    setEmptyColumns(p => p.map((v, idx) => idx === i ? val : v)), [])
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: '100dvh', background: 'rgba(0,0,0,0.55)', zIndex: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ background: 'var(--bg-primary)', borderRadius: '16px', width: '100%', maxWidth: '640px', maxHeight: '92vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '5px' }}>
                <button onClick={selectAll} style={{ fontSize: '11px', padding: '3px 9px', borderRadius: '6px', background: 'var(--brand-light)', border: '1px solid var(--brand)', color: 'var(--brand)', cursor: 'pointer', fontFamily: 'inherit' }}>
                  {isBn ? 'সব' : 'All'}
                </button>
                <button onClick={clearAll} style={{ fontSize: '11px', padding: '3px 9px', borderRadius: '6px', background: 'var(--bg-secondary)', border: '1px solid var(--border)', color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: 'inherit' }}>
                  {isBn ? 'পরিষ্কার' : 'Clear'}
                </button>
              </div>
            </div>
          ...
          <div style={{ marginBottom: '20px', padding: '14px', background: 'var(--bg-secondary)', borderRadius: '12px', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              <div>
                ...
                <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>
                  {isBn ? 'হাতে লেখার জন্য (যেমন: স্বাক্ষর, তারিখ)' : 'For handwriting (e.g. Signature, Date, Remarks)'}
                </div>
              </div>
            ...
            {emptyColumns.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '12px', border: '1px dashed var(--border-2)', borderRadius: '8px', color: 'var(--text-muted)', fontSize: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                ...
                <input type="number" min={0} max={500} value={emptyRows}
                  ...
                  style={{ width: '60px', padding: '5px 8px', borderRadius: '7px', border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '13px', fontFamily: 'inherit', outline: 'none', textAlign: 'center' }} />
              </div>
            </div>
          </div>
        </div>

...
const connectorBase: React.CSSProperties = { flex: 1, height: '2px', minWidth: '20px', maxWidth: '60px', borderRadius: '1px', margin: '0 4px' }
          ...
          return (
            <div
              ...
              style={{
                ...
                cursor: onStepClick ? 'pointer' : 'default', transition: 'all 0.15s',
              }}
            >
              <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontSize: '12px', fontWeight: step.status === 'current' ? 600 : 500,
          ...
          <div key={step.key} style={{ display: 'flex', alignItems: 'center', flex: isLast ? 0 : 1 }}>
            <div
              ...
              style={{
                ...
                cursor: onStepClick ? 'pointer' : 'default', transition: 'all 0.15s', whiteSpace: 'nowrap',
              }}
              onMouseEnter={e => { if (step.status === 'upcoming' && onStepClick) e.currentTarget.style.background = 'var(--bg-secondary)' }}
              onMouseLeave={e => { if (step.status !== 'current') e.currentTarget.style.background = 'transparent' }}
            >
              <div style={{
                ...
                boxShadow: step.status !== 'upcoming' ? `0 0 0 3px ${step.status === 'completed' ? 'rgba(34,197,94,0.15)' : 'rgba(99,102,241,0.15)'}` : 'none',
              }}>
                {step.status === 'completed' ? <Check size={12} /> : <Icon size={12} />}
              </div>
              <div>
                <div style={{ fontSize: '11px', fontWeight: step.status === 'current' ? 600 : 500, color: step.status === 'upcoming' ? 'var(--text-muted)' : 'var(--text-primary)', lineHeight: 1.2 }}>
                  {step.label}
                </div>
            ...
            {!isLast && (
              <div style={{ ...connectorBase, background: step.status === 'completed' ? 'var(--green)' : 'var(--border)' }} />
            )}
          </div>
        )
      })}

...
export const CategoriesTab = ({ searchQuery }: Props) => {
  ...
  const { categories, deleteCategory, toggleCategoryActive } = useExpenseStore()
  ...
  const handleExportExcel = () => {
    ...
    const rows = data.map((c) => ({
      ...
      [bn ? 'ধরন' : 'Type']: c.isSystem ? (bn ? 'সিস্টেম' : 'System') : (bn ? 'কাস্টম' : 'Custom'),
      [bn ? 'স্ট্যাটাস' : 'Status']: c.isActive ? (bn ? 'সক্রিয়' : 'Active') : (bn ? 'নিষ্ক্রিয়' : 'Inactive'),
    }))
    const ws = XLSX.utils.json_to_sheet(rows)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, bn ? 'খরচ ক্যাটাগরি' : 'Expense Categories')
    XLSX.writeFile(wb, `expense-categories-${new Date().toISOString().split('T')[0]}.xlsx`)
  ...
  const buildPdfRow = useCallback((c: (typeof filtered)[number], cols: string[], idx: number): Record<string, string | number> => {
    ...
    if (cols.includes('type')) row[bn ? 'ধরন' : 'Type'] = c.isSystem ? (bn ? 'সিস্টেম' : 'System') : (bn ? 'কাস্টম' : 'Custom')
    return row
  }, [bn])
    ...
    const rows = data.slice(0, 20).map((c, i) => {
      ...
      for (const key of opts.selectedCols) {
        switch (key) {
          ...
          case 'type': row[bn ? 'ধরন' : 'Type'] = c.isSystem ? (bn ? 'সিস্টেম' : 'System') : (bn ? 'কাস্টম' : 'Custom'); break
        }
      }
      return row
    })
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
            ...
            {showActionMenu && (
              <>
                ...
                <div ref={actionMenuRef} className="absolute top-full right-0 mt-1.5 rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] shadow-[0_8px_24px_rgba(0,0,0,0.12)] min-w-[12.5rem] z-50 overflow-hidden">
                  {canExport('finance.expenses.categories') && (
                  ...
                  {canExport('finance.expenses.categories') && <div className="h-px bg-[var(--border)] mx-2" />}
                  {canExport('finance.expenses.categories') && (
          ...
          {canCreate('finance.expenses.categories') && (
                ...
                <tr key={c.id} className={`border-b border-[var(--border)] last:border-0 transition-colors hover:bg-[var(--bg-tertiary)] ${selected.has(c.id) ? 'bg-[var(--brand)]/5' : ''}`}>
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className="flex items-center gap-1 justify-center">
                      {canEdit('finance.expenses.categories') && (
                      ...
                      {canDelete('finance.expenses.categories') && (
      ...
      {deleteId && <DeleteConfirmDialog title={bn ? 'ক্যাটাগরি মুছে ফেলুন?' : 'Delete category?'} message={bn ? 'এই ক্যাটাগরি এবং এর সব খরচ স্থায়ীভাবে মুছে ফেলা হবে।' : 'This category and all its expenses will be permanently deleted.'} onConfirm={() => { deleteCategory(deleteId); setDeleteId(null) }} onCancel={() => setDeleteId(null)} isBn={bn} />}
      {showPdfModal && <GenericPDFOptionsModal columns={pdfColumns} defaultTitle={bn ? 'খরচ ক্যাটাগরি' : 'Expense Categories'} defaultTitleBn="খরচ ক্যাটাগরি" recordLabel={bn ? 'ক্যাটাগরি' : 'category'} recordLabelBn="ক্যাটাগরি" count={selected.size > 0 ? selected.size : filtered.length} isBn={bn} previewRenderer={pdfPreviewRenderer} onDownload={(opts: GenericPDFOptionsResult) => { setShowPdfModal(false); handlePdfDownload(opts) }} onClose={() => setShowPdfModal(false)} />}
    </div>
  )
}

...
export const RecurringTab = ({ searchQuery }: Props) => {
  ...
  const enriched = useMemo(() => {
    return expenses
      .filter((e) => e.isRecurring && e.recurringMonths.length > 0)
      .map((e) => {
  ...
  const handleExportExcel = () => {
    ...
    const rows = data.map((e) => ({
      ...
      [bn ? 'মাস' : 'Months']: e.recurringMonths.map((m) => getMonthName(m, bn)).join(', '),
      [bn ? 'পেমেন্ট' : 'Payment']: e.paymentLabel ? (bn ? e.paymentLabel.labelBn : e.paymentLabel.label) : e.paymentMethod,
    }))
    const ws = XLSX.utils.json_to_sheet(rows)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, bn ? 'পুনরাবৃত্ত খরচ' : 'Recurring Expenses')
    XLSX.writeFile(wb, `recurring-expenses-${new Date().toISOString().split('T')[0]}.xlsx`)
  ...
  const buildPdfRow = useCallback((e: (typeof filtered)[number], cols: string[], idx: number): Record<string, string | number> => {
    ...
    if (cols.includes('months')) row[bn ? 'মাস' : 'Months'] = e.recurringMonths.map((m) => getMonthName(m, bn)).join(', ')
    ...
    const rows = data.slice(0, 20).map((e, i) => {
      ...
      for (const key of opts.selectedCols) {
        switch (key) {
          ...
          case 'months': row[bn ? 'মাস' : 'Months'] = e.recurringMonths.map((m) => getMonthName(m, bn)).join(', '); break
  ...
  const totalRecurringAmount = useMemo(() => filtered.reduce((sum, e) => sum + e.amount, 0), [filtered])
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] text-[var(--text-secondary)]">{bn ? `মোট: ${toBnNum(filtered.length)}টি পুনরাবৃত্ত খরচ (${fmt(totalRecurringAmount)})` : `Total: ${filtered.length} recurring expenses (${fmt(totalRecurringAmount)})`}</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody>
              {paginated.length === 0 ? (
                <tr><td colSpan={9} className="text-center py-12 text-[13px] text-[var(--text-muted)]">{bn ? 'কোনো পুনরাবৃত্ত খরচ পাওয়া যায়নি' : 'No recurring expenses found'}</td></tr>
              ) : paginated.map((e) => (
                <tr key={e.id} className={`border-b border-[var(--border)] last:border-b-0 transition-colors hover:bg-[var(--surface)] ${selected.has(e.id) ? 'bg-[var(--brand)]/5' : ''}`}>
                  ...
                  <td className="py-2.5 px-3 text-center">
                    <div className="flex items-center gap-1 flex-wrap justify-center max-w-[160px] mx-auto">
                      {e.recurringMonths.map((m) => (
                        <span key={m} className="inline-block text-[8px] font-bold px-1 py-0.5 rounded bg-[var(--brand)]/10 text-[var(--brand)]">
                          {getMonthName(m, bn)}
                        </span>
                      ))}
                    </div>
      ...
      {deleteId && <DeleteConfirmDialog title={bn ? 'পুনরাবৃত্ত খরচ মুছে ফেলুন?' : 'Delete recurring expense?'} message={bn ? 'এই পুনরাবৃত্ত খরচটি স্থায়ীভাবে মুছে ফেলা হবে।' : 'This recurring expense will be permanently deleted.'} onConfirm={() => { deleteExpense(deleteId); setDeleteId(null) }} onCancel={() => setDeleteId(null)} isBn={bn} />}
      {showPdfModal && <GenericPDFOptionsModal columns={pdfColumns} defaultTitle={bn ? 'পুনরাবৃত্ত খরচ' : 'Recurring Expenses'} defaultTitleBn="পুনরাবৃত্ত খরচ" recordLabel={bn ? 'পুনরাবৃত্ত খরচ' : 'recurring expense'} recordLabelBn="পুনরাবৃত্ত খরচ" count={selected.size > 0 ? selected.size : filtered.length} isBn={bn} previewRenderer={pdfPreviewRenderer} onDownload={(opts: GenericPDFOptionsResult) => { setShowPdfModal(false); handlePdfDownload(opts) }} onClose={() => setShowPdfModal(false)} />}
    </div>
  )
}

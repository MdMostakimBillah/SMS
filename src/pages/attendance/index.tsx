...
function genSinglePDF(name: string, id: string, photo: string, designation: string, deptName: string, inTime: string, outTime: string, rows: {date:string;status:string;punches:{time:string;type:string}[]}[], isBn: boolean): string {
  ...
  const avgIn = (() => {
    const ins = rows.filter(r => r.status === 'present').map(r => {
  ...
  const trs = rows.map((r, i) => {
    ...
    const lateBadge = isLate ? `<span style="background:#fef3c7;color:#d97706;padding:1px 4px;border-radius:3px;font-size:7px;font-weight:700;margin-left:3px">LATE</span>` : ''
  ...
  const photoTag = photo ? `<img src="${photo}" style="width:48px;height:48px;border-radius:50%;object-fit:cover;border:2px solid #6366f1" />` : `<div style="width:48px;height:48px;border-radius:50%;background:#e0e7ff;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#6366f1;border:2px solid #6366f1">${name.charAt(0)}</div>`
...
export default function AttendancePage() {
  ...
  // Device tab states
  const [devices, setDevices] = useState<{id:string;name:string;model:string;ip:string;status:'online'|'offline'|'error';type:'rfid'|'fingerprint'|'face'|'multi';lastSync:string;staffCount:number}[]>(() => [
    { id:'DEV-001', name:'Main Gate RFID', model:'ZKTeco SLK200', ip:'192.168.1.100', status:'online', type:'rfid', lastSync: new Date().toISOString(), staffCount:18 },
    { id:'DEV-002', name:'Staff Room FP', model:'ZKTeco K40', ip:'192.168.1.101', status:'online', type:'fingerprint', lastSync: new Date().toISOString(), staffCount:15 },
    { id:'DEV-003', name:'Admin Block Face', model:'Hikvision DS-K1T344', ip:'192.168.1.102', status:'offline', type:'face', lastSync: '', staffCount:12 },
    { id:'DEV-004', name:'Back Gate Multi', model:'ZKTeco MB10', ip:'192.168.1.103', status:'online', type:'multi', lastSync: new Date().toISOString(), staffCount:20 },
  ])
  ...
  const [fpEntries, setFpEntries] = useState<{staffId:string;staffName:string;fpId:number;templates:number;status:'enrolled'|'pending'|'failed'}[]>([
  ...
  const [faceEntries, setFaceEntries] = useState<{staffId:string;staffName:string;faceId:number;quality:number;status:'enrolled'|'pending'|'failed'}[]>([
    { staffId:'TCH-2026-001', staffName:'Dr. Rafiqul Islam', faceId:1, quality:92, status:'enrolled' },
    { staffId:'TCH-2026-002', staffName:'Prof. Salma Khatun', faceId:2, quality:88, status:'enrolled' },
    { staffId:'TCH-2026-003', staffName:'Md. Habibur Rahman', faceId:3, quality:85, status:'enrolled' },
    { staffId:'TCH-2026-004', staffName:'Farhana Rahman', faceId:4, quality:90, status:'enrolled' },
    { staffId:'TCH-2026-005', staffName:'Abdul Karim', faceId:5, quality:78, status:'enrolled' },
    { staffId:'TCH-2026-006', staffName:'Nasreen Akhter', faceId:6, quality:82, status:'enrolled' },
    { staffId:'TCH-2026-008', staffName:'Mohammad Ali', faceId:7, quality:0, status:'pending' },
    { staffId:'TCH-2026-009', staffName:'Roksana Begum', faceId:8, quality:0, status:'failed' },
  ])
  ...
  const [deviceSettings, setDeviceSettings] = useState({ autoSync: true, syncInterval: 5, playSound: true })
  ...
  useScrollLock(showMarkAll || viewPerson !== null || viewStudent !== null || showStudentPDF || showEmployeePDF || showAddDevice || showAddRFID || showAddFP || showAddFace || kioskCamActive)
  ...
  const stopKioskCamera = () => {
    if (kioskStreamRef.current) { kioskStreamRef.current.getTracks().forEach(t => t.stop()); kioskStreamRef.current = null }
  ...
  const handleKioskRegister = async () => {
    ...
    if (!kioskCapturedPhoto) {
      setKioskMsg({ type: 'error', text: isBn ? 'প্রথমে ছবি তুলুন।' : 'Take a photo first.' })
      return
    }
    const faceEntry = { staffId: teacher.id, staffName: isBn ? (teacher.nameBn || teacher.nameEn) : teacher.nameEn, photo: kioskCapturedPhoto }
    ...
    setKioskMsg({ type: 'success', text: isBn ? `${faceEntry.staffName} নিবন্ধন সম্পন্ন!` : `${faceEntry.staffName} registered!` })
  ...
  const handleKioskPunch = (staffId: string, staffName: string) => {
    ...
    const status = punchType === 'in' ? 'present' : existing?.status || 'present'
    const updatedAttendance = { ...attendance, [date]: { ...attendance[date], [staffId]: { status: status as AttendanceStatus, punches: punchesNew } } }
    useTeacherStore.setState({ attendance: updatedAttendance })
    setKioskIdentified({ staffId, staffName, punchType, time: timeStr })
    setKioskMsg({ type: 'success', text: `${staffName} ${punchType === 'in' ? (isBn ? 'চেক-ইন' : 'CHECKED IN') : (isBn ? 'চেক-আউট' : 'CHECKED OUT')} ${timeStr}` })
    stopKioskCamera()
    setKioskCapturedPhoto(null)
    setTimeout(() => { setKioskMsg(null); setKioskIdentified(null) }, 4000)
  }
  ...
  const todayFiltered = useMemo(() => {
    const staff = filteredEmployees.map(t => {
      ...
      return { id: t.id, name: isBn ? t.nameBn || t.nameEn : t.nameEn, nameBn: t.nameBn, nameEn: t.nameEn, photo: t.photo, type: 'staff' as const, designation: t.designation || '', dept: getDeptName(t.departmentId), attStatus: status, inTime: inPunch?.time || '—', outTime: outPunch?.time || '—', isLate: !!isLate }
    })
    const stu = filteredStudents.map(s => {
      const rand = Math.random()
      const status: AttendanceStatus = rand < 0.85 ? 'present' : rand < 0.95 ? 'absent' : 'on-leave'
      return { id: s.id, name: isBn ? s.nameBn || s.nameEn : s.nameEn, nameBn: s.nameBn, nameEn: s.nameEn, photo: '', type: 'student' as const, designation: `${s.class} · ${s.section || '—'}`, dept: isBn ? 'শিক্ষার্থী' : 'Student', attStatus: status, inTime: status === 'present' ? '08:00' : '—', outTime: status === 'present' ? '15:00' : '—', isLate: false }
    })
    let l = [...staff, ...stu]
  ...
  const paginatedToday = useMemo(() => todayFiltered.slice((empPage - 1) * empPerPage, empPage * empPerPage), [todayFiltered, empPage, empPerPage])
  ...
  const downloadSinglePDF = useCallback((personId: string, personName: string) => {
    ...
    if (!win) return
    win.document.write(genSinglePDF(
      personName, personId,
      teacher?.photo || '',
      teacher?.designation || '',
...
<div class="ftr"><span>EduTech School Management System</span><div>${optsIsBn?'মুদ্রণ:':'Printed:'} ${new Date().toLocaleDateString()}</div></div></body></html>`
  ...
  const sel = 'px-[9px] py-[7px] rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-secondary)] text-[12px] cursor-pointer outline-none'
  ...
  const statusBadge = (s: AttendanceStatus) => {
    ...
    const x = m[s]
    return <span className="inline-flex items-center justify-center w-5 h-5 rounded-[4px] text-[9px] font-bold" style={{ background:x.bg, color:x.c }}>{x.l}</span>
  }

  const weeklyHolidayBadge = () => {
    return <span className="inline-flex items-center justify-center w-5 h-5 rounded-[4px] text-[9px] font-bold" style={{ background:'var(--purple-light)', color:'var(--purple)' }}>W</span>
  }
  ...
  return (
    <div>
      ...
      {showMarkAll && (
        <div className="fixed inset-0 flex items-center justify-center p-4 z-[700] overflow-y-auto bg-black/50">
          <div className="modal-content bg-[var(--bg-primary)] rounded-[14px] max-w-[380px] w-full p-5 border border-[var(--border)]">
            <h3 className="text-[15px] font-semibold text-[var(--text-primary)] mb-3">{isBn?'সবাইকে উপস্থিত করুন?':'Mark All Present?'}</h3>
            <p className="text-[13px] text-[var(--text-secondary)] mb-4">{isBn?`${activeTeachers.length} জন শিক্ষককে উপস্থিত হিসেবে চিহ্নিত করা হবে।`:`${activeTeachers.length} teachers will be marked present.`}</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowMarkAll(false)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[13px] cursor-pointer">{isBn?'বাতিল':'Cancel'}</button>
              <button onClick={handleMarkAll} className="px-3.5 py-2 rounded-lg bg-[var(--green)] border-0 text-white text-[13px] font-semibold cursor-pointer">{isBn?'নিশ্চিত করুন':'Confirm'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal-content bg-[var(--bg-primary)] rounded-2xl max-w-[650px] w-full max-h-[85vh] overflow-hidden flex flex-col border border-[var(--border)]">
            <div className="px-[18px] py-3.5 border-b border-[var(--border)] flex items-center gap-3 bg-[var(--brand-light)]">
              {(() => {
                ...
                return photoUrl
                  ...
                  : <div className="w-10 h-10 rounded-full bg-[var(--brand-light)] border-2 border-[var(--brand)] flex items-center justify-center text-[var(--brand)] font-bold text-[14px]">{viewPerson.name.charAt(0)}</div>
              })()}
              <div className="flex-1">
                <div className="text-[15px] font-semibold text-[var(--text-primary)]">{viewPerson.name}</div>
                <div className="text-[11px] text-[var(--brand)] font-mono">{viewPerson.id} · {dateFrom} → {dateTo}</div>
              </div>
              <button onClick={() => setViewPerson(null)} className="w-7 h-7 rounded-[7px] bg-[var(--bg-secondary)] border border-[var(--border)] cursor-pointer flex items-center justify-center">
            ...
            <div className="flex-1 overflow-y-auto px-[18px] py-3.5">
              {(() => {
                ...
                const avgIn = (() => {
                  const ins = data.filter(d => d.status === 'present').map(d => {
                  ...
                  <>
                    <div className="grid grid-cols-5 gap-2 mb-3.5">
                      {[
                        ...
                        { l: isBn?'বিলম্ব':'Late', vBn: toBnNum(lateCount), vEn: String(lateCount), c:'var(--red)', bg:'var(--red-light)' },
                        { l: isBn?'গড় ইন':'Avg In', vBn: avgIn, vEn: avgIn, c:'var(--brand)', bg:'var(--brand-light)' },
                      ].map(s => (
                        <div key={s.l} className="p-2 rounded-[8px] text-center" style={{ background:s.bg }}>
                          <div className="text-lg font-bold" style={{ color:s.c }}>{isBn ? s.vBn : s.vEn}</div>
                    ...
                    <table className="w-full border-collapse text-[11px]">
                      <thead>
                        <tr className="bg-[var(--bg-secondary)]">
                          <th className="p-[5px] text-center text-[9px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)] w-[24px]">#</th>
                          <th className="p-[5px] text-left text-[9px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)]">{isBn?'তারিখ':'Date'}</th>
                          <th className="p-[5px] text-center text-[9px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)]">{isBn?'ইন':'In'}</th>
                          <th className="p-[5px] text-center text-[9px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)]">{isBn?'আউট':'Out'}</th>
                          <th className="p-[5px] text-center text-[9px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)]">{isBn?'অবস্থা':'Status'}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.map((d, i) => {
                          ...
                          return (
                            <tr key={d.date} className="border-b border-[var(--border)] hover:bg-[var(--bg-secondary)]">
                              ...
                              <td className="p-[5px] text-center">
                                <span className={`text-[9px] font-mono font-semibold px-[5px] py-[1px] rounded ${
                                  firstIn ? (isLate ? 'bg-[var(--amber-light)] text-[var(--amber)]' : 'bg-[var(--green-light)] text-[var(--green)]') : 'text-[var(--text-muted)]'
                                }`}>{firstIn?.time || '—'}</span>
                              </td>
                              <td className="p-[5px] text-center">
                              ...
                              <td className="p-[5px] text-center">
                                <span className={`text-[9px] font-semibold px-[6px] py-[1px] rounded-[8px] ${
                                  d.status === 'present' ? 'bg-[var(--green-light)] text-[var(--green)]'
                                  : d.status === 'absent' ? 'bg-[var(--red-light)] text-[var(--red)]'
            ...
            <div className="px-[18px] py-3 border-t border-[var(--border)] flex gap-2 justify-end">
              <button onClick={() => setViewPerson(null)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] cursor-pointer">{isBn?'বন্ধ':'Close'}</button>
              <button onClick={() => downloadSinglePDF(viewPerson.id, viewPerson.name)} className="flex items-center gap-[5px] px-3.5 py-2 rounded-lg bg-[var(--red)] border-0 text-white text-[12px] font-semibold cursor-pointer">
          ...
          <div className="modal-content bg-[var(--bg-primary)] rounded-2xl max-w-[600px] w-full max-h-[85vh] overflow-hidden flex flex-col border border-[var(--border)]">
            <div className="px-[18px] py-3.5 border-b border-[var(--border)] flex items-center justify-between bg-[var(--brand-light)]">
              <div>
                ...
                <div className="text-[11px] text-[var(--brand)] font-mono">{viewStudent.id} · {viewStudent.class} · {isBn?'সেকশন':'Section'}: {viewStudent.section}</div>
              </div>
              <button onClick={() => setViewStudent(null)} className="w-7 h-7 rounded-[7px] bg-[var(--bg-secondary)] border border-[var(--border)] cursor-pointer flex items-center justify-center">
                <X size={14} className="text-[var(--text-secondary)]" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto px-[18px] py-3.5">
                    ...
                    <div className="grid grid-cols-5 gap-2 mb-3.5">
                      {[
                        ...
                        <div key={s.l} className="p-2.5 rounded-[10px] text-center" style={{ background:s.bg }}>
                          <div className="text-xl font-bold" style={{ color:s.c }}>{isBn ? s.vBn : s.vEn}</div>
                          <div className="text-[10px]" style={{ color:s.c }}>{s.l}</div>
                        </div>
                      ))}
                    </div>
                    <table className="w-full border-collapse text-[11px]">
                      <thead>
                        <tr className="bg-[var(--bg-secondary)]">
                          <th className="p-[6px] text-left text-[10px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)]">#</th>
                          <th className="p-[6px] text-left text-[10px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)]">{isBn?'তারিখ':'Date'}</th>
                          <th className="p-[6px] text-left text-[10px] font-semibold text-[var(--text-muted)] border-b border-[var(--border)]">{isBn?'অবস্থা':'Status'}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.map((d, i) => (
                          <tr key={d.date} className="border-b border-[var(--border)] hover:bg-[var(--bg-secondary)]">
                            ...
                            <td className="px-2 py-[5px]">
                              {d.isWeeklyHoliday ? (
                                ...
                                <span className={`text-[10px] font-semibold px-[7px] py-[2px] rounded-[10px] ${
                                  d.status === 'present' ? 'bg-[var(--green-light)] text-[var(--green)]'
                                  : d.status === 'absent' ? 'bg-[var(--red-light)] text-[var(--red)]'
                                  : 'bg-[var(--amber-light)] text-[var(--amber)]'
                                }`}>
            ...
            <div className="px-[18px] py-3 border-t border-[var(--border)] flex gap-2 justify-end">
              <button onClick={() => setViewStudent(null)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] cursor-pointer">{isBn?'বন্ধ':'Close'}</button>
              <button onClick={() => downloadStudentSinglePDF(viewStudent.id, viewStudent.name, viewStudent.class, viewStudent.section)} className="flex items-center gap-[5px] px-3.5 py-2 rounded-lg bg-[var(--red)] border-0 text-white text-[12px] font-semibold cursor-pointer">
                <FileText size={13} />PDF
              </button>
            </div>
          </div>
      ...
      <div className={`flex gap-2.5 mb-4 flex-wrap ${isMobile ? 'flex-col items-start' : 'items-center'}`}>
        ...
        <div className="flex-1">
          <h1 className={`font-semibold text-[var(--text-primary)] ${isMobile ? 'text-[18px]' : 'text-[22px]'}`}>{isBn?'উপস্থিতি ব্যবস্থাপনা':'Attendance Management'}</h1>
          <p className="text-[13px] text-[var(--text-secondary)] mt-[3px]">{isBn?'শিক্ষক, কর্মচারী এবং ছাত্রদের উপস্থিতি ট্র্যাক করুন':'Track teacher, employee and student attendance'}</p>
        </div>
        {/* Legend Box */}
        <div className={`bg-[var(--bg-primary)] border border-[var(--border)] rounded-[10px] px-3 py-2 flex flex-wrap gap-[10px] shrink-0 ${isMobile ? 'w-full gap-1.5' : ''}`}>
          {legendItems.map(item => (
            <div key={item.l} className="flex items-center gap-1">
              <span className="inline-flex items-center justify-center w-[18px] h-[18px] rounded-[4px] text-[8px] font-bold" style={{ background:item.bg, color:item.color }}>
                <item.Icon size={10} />
              </span>
              <span className="text-[10px] text-[var(--text-secondary)] font-medium">{item.l}={item.label}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-[10px] text-[12px] cursor-pointer transition-all ${
              activeTab === tab.key ? 'font-semibold' : 'font-normal border border-[var(--border)] bg-[var(--bg-primary)] text-[var(--text-secondary)]'
            }`}
            style={activeTab === tab.key ? { border:`1.5px solid ${tab.color}`, background:`${tab.color}15`, color:tab.color } : {}}>
          ...
          {statusFilters.map(sf => (
            <button key={sf.key} onClick={() => setStatusFilter(sf.key)}
              className={`px-3 py-1 rounded-lg text-[11px] cursor-pointer border ${
                statusFilter === sf.key ? 'font-semibold' : 'font-normal border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-secondary)]'
              }`}
              style={statusFilter === sf.key ? { border:`1px solid ${sf.color}`, background:`${sf.color}18`, color:sf.color } : {}}>
      ...
      {activeTab === 'student' && (
        <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl px-3.5 py-[10px] mb-3.5 flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1.5 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-[9px] py-[5px] flex-1 min-w-[160px]">
            ...
            {studentSearch && <button onClick={() => setStudentSearch('')} className="border-none bg-transparent cursor-pointer text-[var(--text-muted)] flex"><X size={11} /></button>}
          </div>
          <select value={fClass} onChange={e => { setFClass(e.target.value); setFSection('') }} className={sel}>
          ...
          {(fClass || fSection || studentSearch) && <button onClick={() => { setFClass(''); setFSection(''); setStudentSearch('') }} className="px-2 py-[3px] rounded-[6px] bg-[var(--red-light)] border border-[var(--red)] text-[var(--red)] text-[10px] cursor-pointer">✕</button>}
          <div className="flex-1" />
          <CalendarRange size={14} className="text-[var(--text-muted)]" />
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
            className="px-2 py-[5px] rounded-[7px] border border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-primary)] text-[11px] outline-none" />
        ...
        <>
          ...
          <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl p-3.5 mb-3.5">
            <div className="flex items-center gap-2.5 flex-wrap mb-3">
              ...
              <div className="flex-1" />
              <button onClick={() => setShowMarkAll(true)}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {statusFilters.map(sf => (
                <button key={sf.key} onClick={() => { setStatusFilter(sf.key); setEmpPage(1) }}
                  ...
                  style={statusFilter === sf.key ? { borderColor: sf.color, background: `${sf.color}18`, color: sf.color } : {}}>
                  {isBn ? sf.labelBn : sf.labelEn}
                </button>
              ))}
          ...
          <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-[14px] overflow-hidden">
            <div className="overflow-auto max-h-[65vh]">
              <table className="w-full border-collapse text-[11px]">
                <thead>
                  <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                    <th className="p-2 text-center text-[10px] font-semibold text-[var(--text-muted)] w-[36px]">
                      <input type="checkbox" checked={todayFiltered.length > 0 && todayFiltered.every(t => selectedEmployees.includes(t.id))}
                        onChange={() => {
                    ...
                    <th className="p-2 text-left text-[10px] font-semibold text-[var(--text-muted)] min-w-[140px]">{isBn?'নাম':'Name'}</th>
                    <th className="p-2 text-left text-[10px] font-semibold text-[var(--text-muted)] min-w-[80px]">{isBn?'বিভাগ':'Dept'}</th>
                    <th className="p-2 text-left text-[10px] font-semibold text-[var(--text-muted)] min-w-[80px]">{isBn?'পদবি':'Desig'}</th>
                    <th className="p-2 text-center text-[10px] font-semibold text-[var(--text-muted)] min-w-[70px]">
                      <div className="flex items-center justify-center gap-1"><LogOut size={10} />{isBn?'ইন টাইম':'In-Time'}</div>
                    </th>
                    <th className="p-2 text-center text-[10px] font-semibold text-[var(--text-muted)] min-w-[70px]">
                      <div className="flex items-center justify-center gap-1"><LogOut size={10} />{isBn?'আউট টাইম':'Out-Time'}</div>
                    </th>
                    <th className="p-2 text-center text-[10px] font-semibold text-[var(--text-muted)] min-w-[60px]">{isBn?'অবস্থা':'Status'}</th>
                  ...
                  {paginatedToday.map(t => {
                    ...
                    return (
                      <tr key={t.id}
                        ...
                        <td className="p-2">
                          <div className="flex items-center gap-1.5 cursor-pointer" onClick={() => setViewPerson({id:t.id, name:t.name, type:'teacher'})}>
                            <div className="text-[11px] font-medium text-[var(--text-primary)]">{t.name}</div>
                            <ExternalLink size={10} className="text-[var(--text-muted)]" />
                          </div>
                          <div className="flex items-center gap-1">
                            <div className="text-[9px] text-[var(--text-muted)] font-mono">{t.id}</div>
                            <span className={`text-[7px] px-[4px] py-[1px] rounded font-semibold ${
                              t.type === 'staff' ? 'bg-[var(--brand-light)] text-[var(--brand)]' : 'bg-[var(--green-light)] text-[var(--green)]'
                            }`}>{t.type === 'staff' ? 'STAFF' : 'STU'}</span>
                          </div>
                        </td>
                        ...
                        <td className="p-2 text-center">
                          {isPresent && t.inTime !== '—' ? (
                            <span className={`text-[10px] font-mono font-semibold px-2 py-[2px] rounded ${
                        ...
                        <td className="p-2 text-center">
                          <button onClick={() => setViewPerson({id:t.id, name:t.name, type:'teacher'})}
            ...
            <div className="px-3.5 py-2.5 border-t border-[var(--border)] bg-[var(--bg-secondary)] flex justify-between items-center text-[11px] text-[var(--text-muted)]">
              ...
              <span>{todayFiltered.length} {isBn?'জন':'total'} ({filteredEmployees.length} {isBn?'স্টাফ':'staff'} + {filteredStudents.length} {isBn?'শিক্ষার্থী':'students'}) · {new Date(date).toLocaleDateString('en',{weekday:'short', month:'short', day:'numeric'})}</span>
            </div>
            {/* Pagination */}
            <div className="px-3.5 py-2.5 border-t border-[var(--border)] bg-[var(--bg-secondary)] flex justify-between items-center flex-wrap gap-2">
              <div className="flex items-center gap-2">
      ...
      {activeTab === 'range' && (
        <>
          ...
          <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl px-3.5 py-[10px] mb-3.5 flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <span className="text-[12px] text-[var(--text-secondary)]">{isBn?`${filteredEmployees.length} জন কর্মচারী · ${rangeDays.length} দিন`:`${filteredEmployees.length} employees · ${rangeDays.length} days`}</span>
              {selectedEmployees.length > 0 && (
                <span className="text-[11px] text-[var(--brand)] bg-[var(--brand-light)] px-2.5 py-[3px] rounded-[6px] font-medium">
                  {selectedEmployees.length} {isBn?'নির্বাচিত':'selected'}
                </span>
                  ...
                  {paginatedEmployees.map((t) => (
                    <tr key={t.id}
                      ...
                      {rangeDays.map(ds => {
                        ...
                        return <td key={ds} className="p-[4px] text-center">{statusBadge(s)}</td>
                      })}
                    </tr>
                  ))}
                  {filteredEmployees.length === 0 && (
        ...
        <>
          ...
          <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl px-3.5 py-[10px] mb-3.5 flex items-center gap-2 flex-wrap">
            ...
            {[
              ...
              <button key={tab.key} onClick={() => setDeviceTab(tab.key)}
                className={`px-3 py-1 rounded-lg text-[11px] cursor-pointer border ${
                  deviceTab === tab.key ? 'font-semibold' : 'font-normal border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-secondary)]'
                }`}
                style={deviceTab === tab.key ? { border:`1px solid ${tab.color}`, background:`${tab.color}18`, color:tab.color } : {}}>
                <span className="flex items-center gap-1"><tab.Icon size={12} />{isBn ? tab.lBn : tab.lEn}</span>
              </button>
            ))}
            <div className="flex-1" />
            {deviceTab === 'devices' && (
              <button onClick={() => setShowAddDevice(true)} className="flex items-center gap-[5px] px-3 py-[6px] rounded-lg bg-[#7C3AED] text-white text-[11px] cursor-pointer font-medium">
                <Plus size={12} />{isBn?'ডিভাইস যোগ':'Add Device'}
              </button>
            )}
            {deviceTab === 'rfid' && (
              <button onClick={() => setShowAddRFID(true)} className="flex items-center gap-[5px] px-3 py-[6px] rounded-lg bg-[var(--brand)] text-white text-[11px] cursor-pointer font-medium">
                <Plus size={12} />{isBn?'কার্ড যোগ':'Add Card'}
              </button>
            )}
            {deviceTab === 'fingerprint' && (
              <button onClick={() => setShowAddFP(true)} className="flex items-center gap-[5px] px-3 py-[6px] rounded-lg bg-[var(--amber)] text-white text-[11px] cursor-pointer font-medium">
                <Fingerprint size={12} />{isBn?'এনরোল':'Enroll FP'}
              </button>
            )}
            {deviceTab === 'face' && (
          ...
          {deviceTab === 'devices' && (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 mb-4">
                {[
                  { label: isBn?'মোট ডিভাইস':'Total Devices', value: devices.length, icon: <Layers size={16}/>, bg: 'bg-[#7C3AED12]', color: '#7C3AED', border: 'border-[#7C3AED30]' },
                  { label: isBn?'অনলাইন':'Online', value: devices.filter(d=>d.status==='online').length, icon: <Wifi size={16}/>, bg: 'bg-[var(--green-light)]', color: 'var(--green)', border: 'border-[var(--green-light)]' },
                  { label: isBn?'অফলাইন':'Offline', value: devices.filter(d=>d.status==='offline').length, icon: <WifiOff size={16}/>, bg: 'bg-[var(--red-light)]', color: 'var(--red)', border: 'border-[var(--red-light)]' },
                  { label: isBn?'মোট স্টাফ':'Total Staff', value: devices.reduce((s,d)=>s+d.staffCount,0), icon: <Users size={16}/>, bg: 'bg-[var(--brand-light)]', color: 'var(--brand)', border: 'border-[var(--brand-light)]' },
                ].map((s,i) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {devices.map(d => {
                  ...
                  return (
                    <div key={d.id} className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                      {/* Card Header */}
                      <div className="px-4 pt-3.5 pb-2.5" style={{ background:`linear-gradient(135deg, ${typeColor}08 0%, transparent 60%)` }}>
                        <div className="flex items-start justify-between mb-2.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-md" style={{ background:typeColor }}>
                              {d.type==='rfid' ? <CreditCard size={18} className="text-white" /> :
                               d.type==='fingerprint' ? <Fingerprint size={18} className="text-white" /> :
                               <ScanFace size={18} className="text-white" />}
                            </div>
                            <div>
                              <div className="text-[13px] font-bold text-[var(--text-primary)]">{d.name}</div>
                              <div className="text-[10px] text-[var(--text-muted)] mt-0.5">{d.model} · <span className="font-mono">{d.ip}</span></div>
                            </div>
                          </div>
                          <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wide ${
                            d.status==='online'?'bg-[var(--green-light)] text-[var(--green)]':d.status==='offline'?'bg-[var(--red-light)] text-[var(--red)]':'bg-[var(--amber-light)] text-[var(--amber)]'
                          }`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${d.status==='online'?'bg-[var(--green)] animate-pulse':d.status==='offline'?'bg-[var(--red)]':'bg-[var(--amber)]'}`}></span>
                            {d.status}
                          </div>
                        </div>
                        <div className="flex items-center gap-3 text-[10px] text-[var(--text-muted)]">
                          <span className="flex items-center gap-1"><Tag size={10}/> {typeLabel}</span>
                          <span className="flex items-center gap-1"><Users size={10}/> {d.staffCount} {isBn?'স্টাফ':'staff'}</span>
                          <span className="flex items-center gap-1"><Clock size={10}/> {d.lastSync ? new Date(d.lastSync).toLocaleString('en',{hour:'2-digit',minute:'2-digit',hour12:true,day:'2-digit',month:'short'}) : '—'}</span>
                        </div>
                      </div>
                      {/* Card Actions */}
                      <div className="px-4 py-2.5 border-t border-[var(--border)] flex gap-1.5">
                        <button
                          ...
                          {syncingDevice===d.id ? <><Loader size={11} className="animate-spin" />{isBn?'সিঙ্ক...':'Syncing...'}</> : <><RefreshCw size={11} />{isBn?'সিঙ্ক':'Sync'}</>}
                        </button>
                        <button
                          ...
                          <Settings size={11} />{isBn?'সেটিং':'Settings'}
                        </button>
                        <button
                          ...
                          className={`flex items-center justify-center px-2.5 py-2 rounded-lg border text-[10px] cursor-pointer transition-all duration-200 ${
                            d.status==='online' ? 'bg-[var(--red-light)] border-[var(--red)] text-[var(--red)] hover:bg-[var(--red)] hover:text-white hover:shadow-md' : 'bg-[var(--green-light)] border-[var(--green)] text-[var(--green)] hover:bg-[var(--green)] hover:text-white hover:shadow-md'
                          }`}>
                      ...
                      return (
                        <>
                          <div className="flex items-center justify-between mb-4">
                            <h3 className="text-[15px] font-semibold text-[var(--text-primary)]">{isBn?'ডিভাইস সেটিং':'Device Settings'}</h3>
                            <button onClick={() => setShowDeviceSettings(null)} className="w-7 h-7 rounded-[7px] bg-[var(--bg-secondary)] border border-[var(--border)] cursor-pointer flex items-center justify-center"><X size={14} className="text-[var(--text-secondary)]" /></button>
                          </div>
                          ...
                          <div className="space-y-2">
                            <h4 className="text-[11px] font-semibold text-[var(--text-secondary)]">{isBn?'সিঙ্ক সেটিং':'Sync Settings'}</h4>
                            <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--bg-secondary)]">
                              <span className="text-[11px] text-[var(--text-secondary)]">{isBn?'অটো সিঙ্ক':'Auto Sync'}</span>
                              <button onClick={() => setDeviceSettings(s => ({...s, autoSync:!s.autoSync}))}
                                ...
                                <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${deviceSettings.autoSync?'right-0.5':'left-0.5'}`}></span>
                              </button>
                            </div>
                            <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--bg-secondary)]">
                              <span className="text-[11px] text-[var(--text-secondary)]">{isBn?'সিঙ্ক ইন্টারভাল':'Sync Interval'}</span>
                              <select value={deviceSettings.syncInterval} onChange={e => setDeviceSettings(s => ({...s, syncInterval:+e.target.value}))}
                            ...
                            <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--bg-secondary)]">
                              ...
                              <button onClick={() => setDeviceSettings(s => ({...s, playSound:!s.playSound}))}
                                ...
                                <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${deviceSettings.playSound?'right-0.5':'left-0.5'}`}></span>
                              </button>
                            </div>
                            <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--bg-secondary)]">
                          ...
                          <div className="flex gap-2 justify-end mt-5">
                            <button onClick={() => setShowDeviceSettings(null)} className="px-4 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] font-medium cursor-pointer hover:bg-[var(--border)] transition-all">{isBn?'বন্ধ':'Close'}</button>
                            <button onClick={() => { setDevices(prev => prev.map(dev => dev.id===showDeviceSettings ? {...dev, name:d.name} : dev)); setShowDeviceSettings(null) }}
                              className="px-4 py-2 rounded-lg bg-[var(--green)] text-white text-[12px] font-semibold cursor-pointer hover:opacity-90 hover:shadow-md transition-all">{isBn?'সংরক্ষণ':'Save'}</button>
                          </div>
                        </>
                      )
                    })()}
          ...
          {deviceTab === 'rfid' && (
            <>
              ...
              <div className="grid grid-cols-3 gap-2.5 mb-4">
                {[
                  { label: isBn?'মোট কার্ড':'Total Cards', value: rfidEntries.length, icon: <CreditCard size={15}/>, color: 'var(--brand)' },
                  { label: isBn?'বরাদ্ধ':'Assigned', value: rfidEntries.filter(e=>e.assigned).length, icon: <CheckCircle size={15}/>, color: 'var(--green)' },
                  { label: isBn?'অবরাদ্ধ':'Unassigned', value: rfidEntries.filter(e=>!e.assigned).length, icon: <XCircle size={15}/>, color: 'var(--red)' },
                ].map((s,i) => (
                  <div key={i} className="flex items-center gap-2.5 p-3 rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] hover:shadow-md transition-all">
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="flex items-center gap-1.5 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-[6px] flex-1 max-w-[300px]">
                  ...
                  <input value={rfidSearch} onChange={e => setRfidSearch(e.target.value)} placeholder={isBn?'নাম বা কার্ড নম্বর দিয়ে খুঁজুন...':'Search by name or card number...'} className="flex-1 border-none bg-transparent outline-none text-[12px] text-[var(--text-primary)]" />
                </div>
                <span className="text-[11px] text-[var(--text-muted)]">{rfidEntries.filter(e => !rfidSearch || e.staffName.toLowerCase().includes(rfidSearch.toLowerCase()) || e.rfidCard.includes(rfidSearch)).length} {isBn?'টি ফলাফল':'results'}</span>
              </div>
              {/* Table */}
                  ...
                  <table className="w-full border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                        ...
                        <th className="p-2.5 text-left text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'স্টাফ আইডি':'Staff ID'}</th>
                        <th className="p-2.5 text-left text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'নাম':'Name'}</th>
                        <th className="p-2.5 text-left text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'কার্ড নম্বর':'Card Number'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'টাইপ':'Type'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'অবস্থা':'Status'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'অ্যাকশন':'Action'}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rfidEntries.filter(e => !rfidSearch || e.staffName.toLowerCase().includes(rfidSearch.toLowerCase()) || e.rfidCard.includes(rfidSearch)).map((e,i) => (
                        <tr key={e.rfidCard} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-secondary)] transition-colors">
                          ...
                          <td className="p-2.5 font-mono text-[var(--text-primary)] bg-[var(--bg-secondary)] rounded mx-1">{e.rfidCard}</td>
                          <td className="p-2.5 text-center">
                            <select value={e.type} onChange={ev => setRfidEntries(prev => prev.map(x => x.rfidCard===e.rfidCard ? {...x, type:ev.target.value} : x))}
                              ...
                              <option>Admin</option><option>Faculty</option><option>Staff</option>
                            </select>
                          </td>
                          <td className="p-2.5 text-center">
                            <button onClick={() => setRfidEntries(prev => prev.map(x => x.rfidCard===e.rfidCard ? {...x, assigned:!x.assigned} : x))}
                              className={`text-[9px] px-2.5 py-1 rounded-full font-semibold cursor-pointer border transition-all duration-200 ${
                                e.assigned?'bg-[var(--green-light)] border-[var(--green)] text-[var(--green)] hover:bg-[var(--green)] hover:text-white':'bg-[var(--red-light)] border-[var(--red)] text-[var(--red)] hover:bg-[var(--red)] hover:text-white'
                              }`}>
                          ...
                          <td className="p-2.5 text-center">
                            <div className="flex gap-1.5 justify-center">
                              <button onClick={() => setRfidEntries(prev => prev.map(x => x.rfidCard===e.rfidCard ? {...x, rfidCard:`CARD-${String(Math.floor(Math.random()*9000)+1000)}`} : x))}
                                className="px-2.5 py-1 rounded-md bg-[var(--brand-light)] border border-[var(--brand)] text-[var(--brand)] text-[9px] font-semibold cursor-pointer hover:bg-[var(--brand)] hover:text-white transition-all">{isBn?'রি-অ্যাসাইন':'Re-assign'}</button>
                              <button onClick={() => setRfidEntries(prev => prev.filter(x => x.rfidCard!==e.rfidCard))}
                                className="px-2.5 py-1 rounded-md bg-[var(--red-light)] border border-[var(--red)] text-[var(--red)] text-[9px] font-semibold cursor-pointer hover:bg-[var(--red)] hover:text-white transition-all">{isBn?'মুছুন':'Delete'}</button>
                            </div>
                          </td>
                        </tr>
                      ))}
                      {rfidEntries.filter(e => !rfidSearch || e.staffName.toLowerCase().includes(rfidSearch.toLowerCase()) || e.rfidCard.includes(rfidSearch)).length === 0 && (
                        <tr><td colSpan={7} className="p-8 text-center text-[var(--text-muted)] text-[12px]">{isBn?'কোনো কার্ড পাওয়া যায়নি':'No cards found'}</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
          ...
          {deviceTab === 'fingerprint' && (
            <>
              ...
              <div className="grid grid-cols-3 gap-2.5 mb-4">
                {[
                  { label: isBn?'মোট':'Total', value: fpEntries.length, icon: <Fingerprint size={15}/>, color: 'var(--amber)' },
                  { label: isBn?'এনরোলড':'Enrolled', value: fpEntries.filter(e=>e.status==='enrolled').length, icon: <CheckCircle size={15}/>, color: 'var(--green)' },
                  { label: isBn?'বকেয়':'Pending', value: fpEntries.filter(e=>e.status==='pending').length, icon: <Clock size={15}/>, color: 'var(--amber)' },
                ].map((s,i) => (
                  <div key={i} className="flex items-center gap-2.5 p-3 rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] hover:shadow-md transition-all">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background:`${s.color}15`, color:s.color }}>{s.icon}</div>
                    <div>
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="flex items-center gap-1.5 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-[6px] flex-1 max-w-[300px]">
                  <Search size={13} className="text-[var(--text-muted)]" />
                  <input value={fpSearch} onChange={e => setFpSearch(e.target.value)} placeholder={isBn?'নাম বা আইডি দিয়ে খুঁজুন...':'Search by name or ID...'} className="flex-1 border-none bg-transparent outline-none text-[12px] text-[var(--text-primary)]" />
                </div>
                <span className="text-[11px] text-[var(--text-muted)]">{fpEntries.filter(e => !fpSearch || e.staffName.toLowerCase().includes(fpSearch.toLowerCase())).length} {isBn?'টি ফলাফল':'results'}</span>
              </div>
              {/* Table */}
              <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl overflow-hidden">
                <div className="overflow-auto max-h-[55vh]">
                  <table className="w-full border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                        ...
                        <th className="p-2.5 text-left text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'স্টাফ আইডি':'Staff ID'}</th>
                        <th className="p-2.5 text-left text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'নাম':'Name'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'FP আইডি':'FP ID'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'টেমপ্লেট':'Templates'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'অবস্থা':'Status'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'অ্যাকশন':'Action'}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {fpEntries.filter(e => !fpSearch || e.staffName.toLowerCase().includes(fpSearch.toLowerCase())).map((e,i) => (
                        <tr key={e.staffId} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-secondary)] transition-colors">
                          <td className="p-2.5 text-center text-[var(--text-muted)]">{i+1}</td>
                          <td className="p-2.5 font-mono font-semibold text-[var(--brand)]">{e.staffId}</td>
                          <td className="p-2.5 text-[var(--text-primary)] font-medium">{e.staffName}</td>
                          ...
                          <td className="p-2.5 text-center">
                            <div className="flex gap-1 justify-center">
                              {Array.from({length:2}).map((_,si) => (
                                <span key={si} className={`w-3.5 h-3.5 rounded-md ${si<e.templates?'bg-[var(--green)] shadow-sm':'bg-[var(--border)]'}`}></span>
                              ))}
                            </div>
                          </td>
                          <td className="p-2.5 text-center">
                            <span className={`text-[9px] px-2.5 py-1 rounded-full font-semibold ${
                              e.status==='enrolled'?'bg-[var(--green-light)] text-[var(--green)]':e.status==='pending'?'bg-[var(--amber-light)] text-[var(--amber)]':'bg-[var(--red-light)] text-[var(--red)]'
                            }`}>{e.status==='enrolled'?(isBn?'এনরোলড':'Enrolled'):e.status==='pending'?(isBn?'বকেয়':'Pending'):(isBn?'ব্যর্থ':'Failed')}</span>
                          </td>
                          <td className="p-2.5 text-center">
                            <div className="flex gap-1.5 justify-center">
                              <button onClick={() => {
                                setFpEntries(prev => prev.map(x => x.staffId===e.staffId ? {...x, status:'pending' as const, templates:0} : x))
                                setTimeout(() => setFpEntries(prev => prev.map(x => x.staffId===e.staffId ? {...x, status:'enrolled' as const, templates: Math.floor(Math.random()*2)+1} : x)), 2000)
                              ...
                              <button onClick={() => setFpEntries(prev => prev.filter(x => x.staffId!==e.staffId))}
                                className="px-2.5 py-1 rounded-md bg-[var(--red-light)] border border-[var(--red)] text-[var(--red)] text-[9px] font-semibold cursor-pointer hover:bg-[var(--red)] hover:text-white transition-all">{isBn?'মুছুন':'Delete'}</button>
                            </div>
                          </td>
                        </tr>
                      ...
                      {fpEntries.filter(e => !fpSearch || e.staffName.toLowerCase().includes(fpSearch.toLowerCase())).length === 0 && (
                        <tr><td colSpan={7} className="p-8 text-center text-[var(--text-muted)] text-[12px]">{isBn?'কোনো এনরোলমেন্ট পাওয়া যায়নি':'No enrollments found'}</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
          ...
          {deviceTab === 'face' && (
            <>
              ...
              <div className="grid grid-cols-3 gap-2.5 mb-4">
                {[
                  { label: isBn?'মোট':'Total', value: faceEntries.length, icon: <ScanFace size={15}/>, color: 'var(--green)' },
                  { label: isBn?'এনরোলড':'Enrolled', value: faceEntries.filter(e=>e.status==='enrolled').length, icon: <CheckCircle size={15}/>, color: 'var(--green)' },
                  { label: isBn?'গড কোয়ালিটি':'Good Quality', value: faceEntries.filter(e=>e.quality>=80).length, icon: <Fingerprint size={15}/>, color: 'var(--brand)' },
                ].map((s,i) => (
                  <div key={i} className="flex items-center gap-2.5 p-3 rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] hover:shadow-md transition-all">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background:`${s.color}15`, color:s.color }}>{s.icon}</div>
                    <div>
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="flex items-center gap-1.5 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-[6px] flex-1 max-w-[300px]">
                  <Search size={13} className="text-[var(--text-muted)]" />
                  <input value={faceSearch} onChange={e => setFaceSearch(e.target.value)} placeholder={isBn?'নাম বা আইডি দিয়ে খুঁজুন...':'Search by name or ID...'} className="flex-1 border-none bg-transparent outline-none text-[12px] text-[var(--text-primary)]" />
                </div>
                <span className="text-[11px] text-[var(--text-muted)]">{faceEntries.filter(e => !faceSearch || e.staffName.toLowerCase().includes(faceSearch.toLowerCase())).length} {isBn?'টি ফলাফল':'results'}</span>
              </div>
              {/* Table */}
              <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl overflow-hidden">
                <div className="overflow-auto max-h-[55vh]">
                  <table className="w-full border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                        ...
                        <th className="p-2.5 text-left text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'স্টাফ আইডি':'Staff ID'}</th>
                        <th className="p-2.5 text-left text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'নাম':'Name'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'ফেস আইডি':'Face ID'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'কোয়ালিটি':'Quality'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'অবস্থা':'Status'}</th>
                        <th className="p-2.5 text-center text-[10px] font-semibold text-[var(--text-muted)]">{isBn?'অ্যাকশন':'Action'}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {faceEntries.filter(e => !faceSearch || e.staffName.toLowerCase().includes(faceSearch.toLowerCase())).map((e,i) => (
                        <tr key={e.staffId} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-secondary)] transition-colors">
                          <td className="p-2.5 text-center text-[var(--text-muted)]">{i+1}</td>
                          <td className="p-2.5 font-mono font-semibold text-[var(--brand)]">{e.staffId}</td>
                          <td className="p-2.5 text-[var(--text-primary)] font-medium">{e.staffName}</td>
                          ...
                          <td className="p-2.5 text-center">
                            <div className="flex items-center gap-1.5 justify-center">
                              <div className="w-20 h-1.5 rounded-full bg-[var(--border)] overflow-hidden">
                                <div className="h-full rounded-full transition-all duration-700" style={{ width:`${e.quality}%`, background: e.quality>=80?'var(--green)':e.quality>=60?'var(--amber)':'var(--red)' }}></div>
                              </div>
                              <span className="text-[9px] font-bold tabular-nums" style={{ color: e.quality>=80?'var(--green)':e.quality>=60?'var(--amber)':'var(--red)' }}>{e.quality}%</span>
                            </div>
                          </td>
                          <td className="p-2.5 text-center">
                            <span className={`text-[9px] px-2.5 py-1 rounded-full font-semibold ${
                          ...
                          <td className="p-2.5 text-center">
                            <div className="flex gap-1.5 justify-center">
                              <button onClick={() => {
                                setFaceEntries(prev => prev.map(x => x.staffId===e.staffId ? {...x, status:'pending' as const, quality:0} : x))
                                setTimeout(() => setFaceEntries(prev => prev.map(x => x.staffId===e.staffId ? {...x, status:'enrolled' as const, quality: Math.floor(Math.random()*30)+70} : x)), 2000)
                              }} disabled={e.status==='pending'}
                                className={`px-2.5 py-1 rounded-md text-[9px] font-semibold cursor-pointer border transition-all duration-200 ${
                                  e.status==='pending'
                              ...
                              <button onClick={() => setFaceEntries(prev => prev.filter(x => x.staffId!==e.staffId))}
                                className="px-2.5 py-1 rounded-md bg-[var(--red-light)] border border-[var(--red)] text-[var(--red)] text-[9px] font-semibold cursor-pointer hover:bg-[var(--red)] hover:text-white transition-all">{isBn?'মুছুন':'Delete'}</button>
                            </div>
                          </td>
                        </tr>
                      ...
                      {faceEntries.filter(e => !faceSearch || e.staffName.toLowerCase().includes(faceSearch.toLowerCase())).length === 0 && (
                        <tr><td colSpan={7} className="p-8 text-center text-[var(--text-muted)] text-[12px]">{isBn?'কোনো ফেস ডেটা পাওয়া যায়নি':'No face data found'}</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
          ...
          {showAddDevice && (
            <div className="fixed inset-0 flex items-center justify-center p-4 z-[700] overflow-y-auto bg-black/50">
              <div className="modal-content bg-[var(--bg-primary)] rounded-[14px] max-w-[420px] w-full p-5 border border-[var(--border)]">
                <h3 className="text-[15px] font-semibold text-[var(--text-primary)] mb-4">{isBn?'নতুন ডিভাইস যোগ করুন':'Add New Device'}</h3>
                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn?'ডিভাইসের নাম':'Device Name'}</label>
                    <input value={newDevice.name} onChange={e => setNewDevice(p=>({...p,name:e.target.value}))} placeholder="e.g. Main Gate RFID" className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[12px] text-[var(--text-primary)] outline-none" />
                  </div>
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn?'মডেল':'Model'}</label>
                    <input value={newDevice.model} onChange={e => setNewDevice(p=>({...p,model:e.target.value}))} placeholder="e.g. ZKTeco SLK200" className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[12px] text-[var(--text-primary)] outline-none" />
                  </div>
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">IP {isBn?'ঠিকানা':'Address'}</label>
                    <input value={newDevice.ip} onChange={e => setNewDevice(p=>({...p,ip:e.target.value}))} placeholder="192.168.1.100" className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[12px] text-[var(--text-primary)] font-mono outline-none" />
                  </div>
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn?'ডিভাইস টাইপ':'Device Type'}</label>
                    <div className="grid grid-cols-4 gap-1.5">
                      {[{k:'rfid' as const,l:'RFID',I:CreditCard},{k:'fingerprint' as const,l:'FP',I:Fingerprint},{k:'face' as const,l:'Face',I:ScanFace},{k:'multi' as const,l:'Multi',I:Layers}].map(o => (
                        <button key={o.k} onClick={() => setNewDevice(p=>({...p,type:o.k}))}
                ...
                <div className="flex gap-2 justify-end mt-4">
                  <button onClick={() => setShowAddDevice(false)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] cursor-pointer">{isBn?'বাতিল':'Cancel'}</button>
                  <button onClick={() => {
                  ...
                  }} className="px-3.5 py-2 rounded-lg bg-[#7C3AED] border-0 text-white text-[12px] font-semibold cursor-pointer">{isBn?'যোগ করুন':'Add Device'}</button>
                </div>
              </div>
            </div>
          )}
          ...
          {showAddRFID && (
            <div className="fixed inset-0 flex items-center justify-center p-4 z-[700] overflow-y-auto bg-black/50">
              <div className="modal-content bg-[var(--bg-primary)] rounded-[14px] max-w-[380px] w-full p-5 border border-[var(--border)]">
                <h3 className="text-[15px] font-semibold text-[var(--text-primary)] mb-4">{isBn?'RFID কার্ড যোগ করুন':'Add RFID Card'}</h3>
                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn?'স্টাফ নির্বাচন করুন':'Select Staff'}</label>
                    <select value={newRFID.staffId} onChange={e => setNewRFID(p=>({...p,staffId:e.target.value}))} className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[12px] text-[var(--text-secondary)] outline-none">
                      <option value="">{isBn?'নির্বাচন করুন...':'Select...'}</option>
                      {activeTeachers.map(t => <option key={t.id} value={t.id}>{t.id} - {isBn?t.nameBn||t.nameEn:t.nameEn}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn?'কার্ড নম্বর (বা ট্যাপ করুন)':'Card Number (or tap)'}</label>
                    <input value={newRFID.rfidCard} onChange={e => setNewRFID(p=>({...p,rfidCard:e.target.value}))} placeholder="CARD-XXXX" className="w-full px-3 py-2 rounded-lg border-2 border-dashed border-[var(--border)] bg-[var(--bg-secondary)] text-[12px] text-[var(--text-primary)] font-mono text-center outline-none focus:border-[var(--brand)]" />
                    <p className="text-[9px] text-[var(--text-muted)] mt-1 text-center">{isBn?'ডিভাইসে কার্ড ট্যাপ করুন অথবা ম্যানুয়ালি লিখুন':'Tap card on device or enter manually'}</p>
                  </div>
                </div>
                <div className="flex gap-2 justify-end mt-4">
                  <button onClick={() => setShowAddRFID(false)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] cursor-pointer">{isBn?'বাতিল':'Cancel'}</button>
                  <button onClick={() => {
                    if(newRFID.staffId && newRFID.rfidCard) {
                      const t = activeTeachers.find(te=>te.id===newRFID.staffId)
                      if(t) setRfidEntries(p=>[...p, { staffId:t.id, staffName:isBn?t.nameBn||t.nameEn:t.nameEn, rfidCard:newRFID.rfidCard, type:'Staff', assigned:true }])
                      setShowAddRFID(false)
                      setNewRFID({staffId:'',rfidCard:''})
                    }
                  }} className="px-3.5 py-2 rounded-lg bg-[var(--brand)] border-0 text-white text-[12px] font-semibold cursor-pointer">{isBn?'যোগ করুন':'Add'}</button>
                </div>
              </div>
            </div>
          )}
          ...
          {showAddFP && (
            <div className="fixed inset-0 flex items-center justify-center p-4 z-[700] overflow-y-auto bg-black/50">
              <div className="modal-content bg-[var(--bg-primary)] rounded-[14px] max-w-[380px] w-full p-5 border border-[var(--border)]">
                <h3 className="text-[15px] font-semibold text-[var(--text-primary)] mb-4">{isBn?'ফিঙ্গারপ্রিন্ট এনরোলমেন্ট':'Fingerprint Enrollment'}</h3>
                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn?'স্টাফ নির্বাচন করুন':'Select Staff'}</label>
                    <select value={newFP.staffId} onChange={e => setNewFP(p=>({...p,staffId:e.target.value}))} className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[12px] text-[var(--text-secondary)] outline-none">
                      <option value="">{isBn?'নির্বাচন করুন...':'Select...'}</option>
                      {activeTeachers.map(t => <option key={t.id} value={t.id}>{t.id} - {isBn?t.nameBn||t.nameEn:t.nameEn}</option>)}
                    </select>
                  </div>
                ...
                <div className="flex gap-2 justify-end mt-4">
                  <button onClick={() => setShowAddFP(false)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] cursor-pointer">{isBn?'বাতিল':'Cancel'}</button>
                  <button onClick={() => {
                    if(newFP.staffId) {
                      const t = activeTeachers.find(te=>te.id===newFP.staffId)
                      if(t) setFpEntries(p=>[...p, { staffId:t.id, staffName:isBn?t.nameBn||t.nameEn:t.nameEn, fpId:p.length+1, templates:0, status:'pending' }])
                      setShowAddFP(false)
                      setNewFP({staffId:''})
                    }
                  }} className="px-3.5 py-2 rounded-lg bg-[var(--amber)] border-0 text-white text-[12px] font-semibold cursor-pointer">{isBn?'এনরোল শুরু করুন':'Start Enrollment'}</button>
                </div>
              </div>
            </div>
          )}
          ...
          {showAddFace && (
            <div className="fixed inset-0 flex items-center justify-center p-4 z-[700] overflow-y-auto bg-black/50">
              <div className="modal-content bg-[var(--bg-primary)] rounded-[14px] max-w-[380px] w-full p-5 border border-[var(--border)]">
                <h3 className="text-[15px] font-semibold text-[var(--text-primary)] mb-4">{isBn?'ফেস স্ক্যান এনরোলমেন্ট':'Face Scan Enrollment'}</h3>
                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn?'স্টাফ নির্বাচন করুন':'Select Staff'}</label>
                    <select value={newFace.staffId} onChange={e => setNewFace(p=>({...p,staffId:e.target.value}))} className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-[12px] text-[var(--text-secondary)] outline-none">
                      <option value="">{isBn?'নির্বাচন করুন...':'Select...'}</option>
                      {activeTeachers.map(t => <option key={t.id} value={t.id}>{t.id} - {isBn?t.nameBn||t.nameEn:t.nameEn}</option>)}
                    </select>
                  </div>
                ...
                <div className="flex gap-2 justify-end mt-4">
                  <button onClick={() => setShowAddFace(false)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] cursor-pointer">{isBn?'বাতিল':'Cancel'}</button>
                  <button onClick={() => {
                    if(newFace.staffId) {
                      const t = activeTeachers.find(te=>te.id===newFace.staffId)
                      if(t) setFaceEntries(p=>[...p, { staffId:t.id, staffName:isBn?t.nameBn||t.nameEn:t.nameEn, faceId:p.length+1, quality:0, status:'pending' }])
              ...
              <div className="bg-[var(--bg-primary)] rounded-none sm:rounded-2xl w-full sm:max-w-[420px] min-h-[100dvh] sm:min-h-0 sm:h-auto sm:my-auto p-5 sm:p-6 border-0 sm:border border-[var(--border)] shadow-2xl">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                   ...
                   <button onClick={() => { setKioskMode(false); setKioskMsg(null); setKioskIdentified(null) }}
                ...
                {kioskIdentified && (
                  <div className="mb-4 p-5 rounded-2xl bg-[var(--green-light)] border-2 border-[var(--green)] text-center">
                    ...
                    <div className="mt-3 flex items-center justify-center gap-3">
                      <span className={`px-3 py-1.5 rounded-lg text-[13px] font-bold ${kioskIdentified.punchType === 'in' ? 'bg-[var(--green)] text-white' : 'bg-[var(--amber)] text-white'}`}>
                ...
                {kioskCamActive && !kioskIdentified && (
                  <div className="mb-4">
                    ...
                    {kioskCapturedPhoto ? (
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 max-h-[200px] overflow-y-auto">
                          {kioskRegisteredFaces.map(f => (
                            <button key={f.staffId} onClick={() => handleKioskPunch(f.staffId, f.staffName)}
                              className="flex items-center gap-2 p-2 rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] hover:border-[var(--green)] hover:bg-[var(--green-light)] cursor-pointer transition-all text-left">
                              <img src={f.photo} alt="" className="w-8 h-8 rounded-full object-cover border border-[var(--border)]" />
                              <div>
                                <div className="text-[11px] font-medium text-[var(--text-primary)]">{f.staffName}</div>
                                <div className="text-[9px] text-[var(--text-muted)]">{f.staffId}</div>
                              </div>
                            </button>
                          ))}
                        </div>
                        {kioskRegisteredFaces.length === 0 && (
                          <p className="text-[11px] text-center text-[var(--text-muted)]">{isBn ? 'কোনো নিবন্ধিত স্টাফ নেই। প্রথমে নিবন্ধন করুন।' : 'No registered staff. Register first.'}</p>
                        )}
                        <button onClick={() => { setKioskCapturedPhoto(null) }}
                      ...
                      <button onClick={() => { const photo = captureKioskPhoto(); if (photo) setKioskCapturedPhoto(photo) }}
                    ...
                    <button onClick={() => { stopKioskCamera(); setKioskCapturedPhoto(null) }}
                ...
                {/* Scan / Retry button */}
                {!kioskCamActive && !kioskIdentified && (
                  <button onClick={handleKioskScan} disabled={kioskPending}
                    ...
                    <span className="flex items-center justify-center gap-2"><ScanFace size={18} />{isBn ? 'ক্যামেরা খুলুন' : 'Open Camera'}</span>
                  </button>
                )}

                {kioskIdentified && (
                  <button onClick={() => { setKioskIdentified(null); setKioskMsg(null) }}
                    ...
                    <span className="flex items-center justify-center gap-2"><ScanFace size={18} />{isBn ? 'আবার স্ক্যান করুন' : 'Scan Again'}</span>
                  </button>
                )}
                ...
                <div className="mt-4">
                  {!kioskRegMode ? (
                    <button onClick={() => { setKioskRegMode(true); setKioskRegStaff(''); setKioskMsg(null); setKioskCapturedPhoto(null) }}
                    ...
                    <div className="p-4 rounded-xl border-2 border-[var(--teal)] bg-[var(--teal-light)]">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => { setKioskRegMode(false); setKioskRegStaff(''); setKioskCapturedPhoto(null); stopKioskCamera() }}
                          className="text-[11px] text-[var(--text-muted)] hover:text-[var(--text-primary)] cursor-pointer bg-transparent border-none underline">
                          {isBn ? 'বাতিল' : 'Cancel'}
                        </button>
                      </div>
                      <select value={kioskRegStaff} onChange={e => { setKioskRegStaff(e.target.value); setKioskMsg(null); setKioskCapturedPhoto(null) }}
                      ...
                      {kioskRegStaff && (
                        <>
                          {kioskCamActive ? (
                              ...
                              {kioskCapturedPhoto ? (
                                <div className="space-y-2">
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <button onClick={handleKioskRegister}
                            ...
                            <button onClick={startKioskCamera}
                              ...
                              <ScanFace size={16} />{isBn ? 'ক্যামেরা খুলুন' : 'Open Camera'}
                            </button>
                          )}
                        </>
                      )}
                ...
                <div className="mt-4 flex items-center justify-center gap-2">
                  {wifiConnected === true ? (
                    <span className="text-[10px] text-[var(--green)] flex items-center gap-1"><Wifi size={12} />{isBn?'নেটওয়ার্ক সংযুক্ত':'Network connected'}</span>
                  ) : wifiConnected === false ? (
                    <span className="text-[10px] text-[var(--red)] flex items-center gap-1"><WifiOff size={12} />{isBn?'নেটওয়ার্ক সংযোগ নেই':'No network'}</span>
                  ) : null}
                </div>
      ...
      {activeTab === 'student' && (
        <>
          <div className="flex items-center justify-between mb-[10px] flex-wrap gap-[10px]">
            <div className="flex items-center gap-2.5">
              <span className="text-[12px] text-[var(--text-secondary)]">{isBn?`মোট ${filteredStudents.length} জন শিক্ষার্থী`:`${filteredStudents.length} students`}</span>

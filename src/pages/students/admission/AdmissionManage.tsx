...
import { useAdmissionActions } from './hooks/useAdmissionActions'
...
export default function AdmissionManage() {
  ...
  const students = useMemo(() => allStudents.filter((s) => s.academicYear === currentSession), [allStudents, currentSession])
  const isBn = useBn()
  const { handleApprove, handleReject, exportExcel, handleListPDF } = useAdmissionActions()
  ...
  const filtered = useMemo(
    () => students.filter((s) => {
      if (search) {
        ...
        if (!s.nameEn.toLowerCase().includes(q) && !s.nameBn.includes(search) && !s.id.includes(search) && !s.phone.includes(search)) return false
      }
  ...
  const clearFilters = useCallback(() => {
    setSearch(''); setFClass(''); setFSection(''); setFGender('')
    setFReligion(''); setFStatus(''); setFDate(''); setDateFrom(''); setDateTo(''); setPage(1)
  }, [])
  ...
  return (
    <div>
      {approvingStudent && (
        <ApproveModal student={approvingStudent} isBn={isBn} onClose={() => setApprovingStudent(null)}
          onApprove={(sms, billingDate) => handleApprove(approvingStudent, sms, billingDate)} />
      )}
      {rejectingStudent && (
        <RejectModal student={rejectingStudent} isBn={isBn} onClose={() => setRejectingStudent(null)}
          onReject={(sms) => handleReject(rejectingStudent, sms)} />
      )}
      {editingStudent && (
        <EditModal student={editingStudent} isBn={isBn} onClose={() => setEditingStudent(null)}
          onSave={(d) => updateStudent(editingStudent.id, d)} />
      )}
      {viewingStudent && (
        <ViewModal student={viewingStudent} isBn={isBn} onClose={() => setViewingStudent(null)} teacherMap={teacherMap} />
      )}
      {showPDFModal && (
        <PDFOptionsModal count={selected.length > 0 ? selected.length : filtered.length} isBn={isBn}
          students={selected.length > 0 ? filtered.filter((s) => selected.includes(s.id)) : filtered}
          teachers={teachers} onClose={() => setShowPDFModal(false)}
          onDownload={(opts) => handleListPDF(selected, filtered, opts, () => setShowPDFModal(false))} />
      )}

      <StatsBar stats={stats} isBn={isBn} isMobile={isMobile} />
      <FilterBar search={search} setSearch={setSearch} fClass={fClass} setFClass={setFClass}
        fSection={fSection} setFSection={setFSection} fGender={fGender} setFGender={setFGender}
        fReligion={fReligion} setFReligion={setFReligion} fStatus={fStatus} setFStatus={setFStatus}
        fDate={fDate} setFDate={setFDate} dateFrom={dateFrom} setDateFrom={setDateFrom}
        dateTo={dateTo} setDateTo={setDateTo} classOptions={classOptions} sectionsMap={sectionsMap}
        allSections={allSections} setPage={setPage} clearFilters={clearFilters} hasFilter={hasFilter}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem', flexWrap: 'wrap', gap: '0.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          ...
          <select value={perPage} onChange={(e) => { setPerPage(Number(e.target.value)); setPage(1) }}
            style={{ padding: '5px 8px', borderRadius: '0.5rem', border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-secondary)', fontSize: '0.75rem', fontFamily: 'inherit', cursor: 'pointer', outline: 'none' }}>
            {PER_PAGE.map((n) => <option key={n} value={n}>{n}</option>)}
          </select>
          {selected.length > 0 && (
            <span style={{ fontSize: '0.75rem', color: 'var(--brand)', background: 'var(--brand-light)', padding: '3px 10px', borderRadius: '0.375rem', fontWeight: 500 }}>
              {selected.length} {isBn ? 'টি নির্বাচিত' : 'selected'}
            </span>
          )}
        </div>
        <ActionMenu showActionMenu={showActionMenu} setShowActionMenu={setShowActionMenu} actionMenuRef={actionMenuRef}
          exportExcel={() => exportExcel(selected, filtered)} setShowPDFModal={setShowPDFModal} isBn={isBn} />
      </div>

      <AdmissionTable paginated={paginated} sp={sp} page={sp} perPage={perPage} filtered={filtered} selected={selected}
        allSel={allSel} totalPages={totalPages} toggleAll={toggleAll} toggleOne={toggleOne} setPage={setPage}
        setViewingStudent={setViewingStudent} setEditingStudent={setEditingStudent}
        setApprovingStudent={setApprovingStudent} setRejectingStudent={setRejectingStudent} isBn={isBn} />
    </div>
  )
}

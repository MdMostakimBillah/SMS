...
export const DuesTab = React.memo(function DuesTab({ onCollect }: Props) {
  ...
  const dues = useMemo(() => calculateDues(activeStudents, fClass || undefined, fSection || undefined), [activeStudents, fClass, fSection, structures, payments, waivers])
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[
          ...
          { label: bn ? 'বকেয় শিক্ষার্থী' : 'Students with Dues', value: String(studentCount), color: 'var(--amber)', icon: <Users size={16} /> },
        ].map((s, i) => (
      ...
      <div className="border border-[var(--border)] rounded-xl overflow-hidden">
        {filteredDues.length === 0 ? (
          <div className="text-center py-12 text-[var(--text-muted)] text-sm">
            {bn ? 'কোনো বকেয় নেই' : 'No dues found'}
          </div>
        ) : (
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {filteredDues.map((d, i) => (
                  <tr key={`${d.studentId}-${d.feeStructureId}-${i}`} className="border-t border-[var(--border)] hover:bg-[var(--bg-secondary)]/50 transition-colors">
                    ...
                    <td className="px-3 py-2 text-right font-semibold text-[var(--amber)]">{fmt(d.dueAmount)}</td>
                    <td className="px-3 py-2 text-right">
                      <button onClick={() => onCollect(d)} className="text-[0.65rem] font-medium px-2 py-1 rounded bg-[var(--green-light)] text-[var(--green)] border-0 cursor-pointer hover:bg-[var(--green)] hover:text-white transition-colors">
                        {bn ? 'পরিশোধ' : 'Collect'}
                      </button>
                    </td>
                  </tr>

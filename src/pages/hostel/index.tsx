...
type View = 'rooms' | 'students'

function StatCards({ stats, bn }: { stats: { totalRooms: number; activeRooms: number; assignedStudents: number; totalBeds: number }; bn: boolean }) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {[
        { labelBn: 'মোট রুম', labelEn: 'Total Rooms', value: stats.totalRooms, icon: <Home size={14} />, color: 'var(--brand)' },
        { labelBn: 'সক্রিয় রুম', labelEn: 'Active Rooms', value: stats.activeRooms, icon: <Home size={14} />, color: 'var(--teal)' },
        { labelBn: 'বরাদ্দ ছাত্র', labelEn: 'Assigned Students', value: stats.assignedStudents, icon: <Users size={14} />, color: 'var(--amber)' },
        { labelBn: 'মোট আসন', labelEn: 'Total Beds', value: stats.totalBeds, icon: <BedDouble size={14} />, color: 'var(--green)' },
      ].map((s) => (
...
export default function HostelPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalBeds = rooms.filter((r) => r.isActive).reduce((sum, r) => sum + r.capacity, 0)
    return { totalRooms, activeRooms, assignedStudents, totalBeds }
  }, [rooms, assignments])
    ...
    <div className="space-y-4">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-[var(--text-primary)]">
          {bn ? 'হোস্টেল ব্যবস্থাপনা' : 'Hostel Management'}
        </h1>
      </div>

    ...
    return promoStudents
      .map((student) => {
        const subjectResults = examSubjects.map((sc) => {
          const mark = sessionStudentMarks.find(
            (m) =>
              ...
              m.classId === promoClassId &&
              m.sectionId === promoSectionId
          )
        ...
        return {
          ...
          passedAll,
          grade,
          status: existingPromo?.status || (passedAll ? 'eligible' : 'not-eligible'),
        }
      })
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-[0.625rem] text-[var(--text-muted)] flex-wrap">
                          ...
                          <span>
                            {isBn ? 'সেশন' : 'Session'}: {ms.sessionId}
                          </span>
                          <span>
                            {isBn ? 'কার্যদিবস' : 'Working Days'}: {ms.workingDays}
                          </span>
                        </div>
                      </div>
          ...
          <>
            ...
            <div className={sectionCls}>
              <div className={sectionTitleCls}>
                ...
                {isBn ? 'প্রমোশন নির্বাচন' : 'Promotion Selection'}
              </div>
              <div className="grid grid-cols-3 gap-3 mt-3">
                ...
                <div>
                  ...
                  <select
                    value={promoClassId}
                    onChange={(e) => {
                      setPromoClassId(e.target.value)
                      setPromoSectionId('')
                    }}
            ...
            {promoStats.eligible > 0 && promoStats.promoted === 0 && (
              <div className="mb-3">
                <button onClick={() => setShowPromoConfirm(true)} className={`${btnPrimary}`}>
                  <GraduationCap size={14} />
                  {isBn ? `${promoStats.eligible} জনকে প্রমোট করুন` : `Promote ${promoStats.eligible} Students`}
                </button>
              </div>
            )}
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setMarksheetForm((p) => ({ ...p, name: e.target.value }))}
                    className={`${inputCls} w-full`}
                  />
                </div>
                <div>
                  ...
                  <input
                    value={marksheetForm.nameBn}
                    onChange={(e) => setMarksheetForm((p) => ({ ...p, nameBn: e.target.value }))}
                    className={`${inputCls} w-full`}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={marksheetForm.workingDays}
                    onChange={(e) => setMarksheetForm((p) => ({ ...p, workingDays: e.target.value }))}
                    className={`${inputCls} w-full`}
                  />
                </div>
              </div>
              <div>
                ...
                <input
                  value={marksheetForm.schoolName}
                  onChange={(e) => setMarksheetForm((p) => ({ ...p, schoolName: e.target.value }))}
                  className={`${inputCls} w-full`}
                />
              </div>
            </div>

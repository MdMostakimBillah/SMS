...
import {
  ArrowLeft, Plus, Trash2, BarChart2, Award,
  Users, CheckCircle, AlertTriangle, TrendingUp,
  Target, FileSpreadsheet,
} from 'lucide-react'
...
export default function Step4Results() {
  ...
  const [extraForm, setExtraForm] = useState({ studentId: '', type: 'attendance' as 'attendance' | 'discipline' | 'homework', marks: '0', maxMarks: '10', note: '' })
    ...
    return classStudents.map(student => {
      const subjectMarks = examSubjects.map(sc => {
        const mark = studentMarks.find(m =>
          ...
          m.subjectId === sc.subjectId && m.classId === selectedClassId && m.sectionId === selectedSectionId
        )
      ...
      const gpa = passedAll ? (percentage >= 80 ? 5.0 : percentage >= 70 ? 4.0 : percentage >= 60 ? 3.5 : percentage >= 50 ? 3.0 : percentage >= 40 ? 2.0 : percentage >= 33 ? 1.0 : 0) : 0
  ...
  const analysisData = useMemo(() => {
    ...
    const subjectStats = examSubjects.map(sc => {
      ...
      return { subjectId: sc.subjectId, subjectName: isBn ? subject?.nameBn : subject?.name, fullMarks: sc.fullMarks, total, avg, passRate, highest, lowest }
    })
  ...
  const handleSaveExtra = () => {
    ...
    addExtraMark({
      examId: selectedExamId, studentId: extraForm.studentId,
      classId: selectedClassId, sectionId: selectedSectionId,
      type: extraForm.type, marks: Number(extraForm.marks) || 0,
      maxMarks: Number(extraForm.maxMarks) || 10, note: extraForm.note,
    })
    ...
    <div className="flex flex-col h-[calc(100vh-60px)]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)] bg-[var(--bg-primary)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[16px] font-bold text-[var(--text-primary)]">{isBn ? 'ধাপ ৪: ফলাফল প্রক্রিয়াকরণ' : 'Step 4: Result Processing'}</h1>
            <p className="text-[11px] text-[var(--text-muted)]">{isBn ? 'এক্সট্রা মার্কস, ট্যাবুলেশন ও বিশ্লেষণ' : 'Extra marks, tabulation & analysis'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-2 border-b border-[var(--border)] bg-[var(--bg-primary)] flex gap-2 items-center flex-wrap">
        ...
        <select value={selectedClassId} onChange={e => { setSelectedClassId(e.target.value); setSelectedSectionId('') }} className={`${inputCls} max-w-[140px]`}>
          <option value="">{isBn ? 'শ্রেণি...' : 'Class...'}</option>
          {classOptions.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={selectedSectionId} onChange={e => setSelectedSectionId(e.target.value)} className={`${inputCls} max-w-[140px]`} disabled={!selectedClassId}>
      ...
      <div className="flex-1 overflow-y-auto p-4">
            ...
            {tabulationData.length > 0 ? (
              <div className={sectionCls}>
                <div className={sectionTitleCls}>
                  <FileSpreadsheet size={15} className="text-[var(--brand)]" />{isBn ? 'ট্যাবুলেশন শিট' : 'Tabulation Sheet'} ({tabulationData.length} {isBn ? 'জন' : 'students'})
                </div>
                    ...
                    <tbody>
                      {tabulationData.map((row, idx) => (
                        <tr key={row.student.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-secondary)]">
                          ...
                          <td className="py-2 px-2 text-[11px] font-medium text-[var(--text-primary)]">{isBn ? row.student.nameBn || row.student.nameEn : row.student.nameEn}</td>
                          {row.subjectMarks.map(sm => (
                            <td key={sm.subjectId} className="py-2 px-2 text-center">
                              <span className={`text-[11px] font-medium ${sm.passed ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}>{sm.obtained}</span>
                          ...
                          <td className="py-2 px-2 text-center">
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${row.gpa >= 4 ? 'bg-[#dcfce7] text-[#15803d]' : row.gpa >= 2 ? 'bg-[#dbeafe] text-[#1d4ed8]' : 'bg-[var(--red-light)] text-[var(--red)]'}`}>
                              {row.gpa.toFixed(1)}
                            </span>
                          </td>
                          <td className="py-2 px-2 text-center">
                            <span className={`text-[10px] font-medium px-2 py-0.5 rounded ${row.passedAll ? 'bg-[var(--green-light)] text-[var(--green)]' : 'bg-[var(--red-light)] text-[var(--red)]'}`}>
              ...
              <div className={`${sectionCls} text-center py-10`}>
                ...
                <p className="text-[13px] text-[var(--text-muted)]">{isBn ? 'পরীক্ষা, শ্রেণি ও সেকশন নির্বাচন করুন' : 'Select exam, class and section to view tabulation'}</p>
              </div>
            )}
          </>
        )}
        ...
        {activeSubTab === 'extra-marks' && (
          <>
            <div className="flex justify-between items-center mb-3">
              <span className="text-[12px] text-[var(--text-secondary)]">{extraMarks.filter(e => selectedExamId ? e.examId === selectedExamId : true).length} {isBn ? 'টি এন্ট্রি' : 'entries'}</span>
              <button onClick={() => setShowExtraForm(true)} className={btnPrimary}><Plus size={14} />{isBn ? 'নতুন এন্ট্রি' : 'New Entry'}</button>
            </div>
                  ...
                  <div key={type.key} className={sectionCls} style={{ borderColor: `${type.color}30` }}>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-[var(--text-muted)]">{count} {isBn ? 'টি এন্ট্রি' : 'entries'}</div>
                      </div>
                    </div>
                  </div>
                )
            ...
            <div className={sectionCls}>
              <div className="space-y-2">
                {extraMarks.filter(e => selectedExamId ? e.examId === selectedExamId : true).map(entry => {
                  ...
                  return (
                    <div key={entry.id} className="flex items-center gap-3 p-3 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-[var(--text-muted)]">{entry.note} · {entry.classId} {entry.sectionId}</div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-[13px] font-bold text-[var(--text-primary)]">{entry.marks}/{entry.maxMarks}</div>
                      </div>
                      <button onClick={() => { if (confirm(isBn ? 'মুছে ফেলবেন?' : 'Delete?')) deleteExtraMark(entry.id) }}
        ...
        {activeSubTab === 'analysis' && analysisData && (
          <>
            <div className="grid grid-cols-3 gap-3 mb-3">
              {[
                { label: isBn ? 'মোট শিক্ষার্থী' : 'Total Students', value: analysisData.totalStudents, color: 'var(--brand)', icon: <Users size={16} /> },
                { label: isBn ? 'পাস হার' : 'Pass Rate', value: `${analysisData.passRate}%`, color: 'var(--green)', icon: <CheckCircle size={16} /> },
                { label: isBn ? 'গড় শতাংশ' : 'Avg Percentage', value: `${analysisData.avgPercentage}%`, color: 'var(--amber)', icon: <TrendingUp size={16} /> },
              ].map(s => (
            ...
            <div className={sectionCls}>
              <div className={sectionTitleCls}>
                <BarChart2 size={15} className="text-[var(--brand)]" />{isBn ? 'বিষয়ভিত্তিক পারফরম্যান্স' : 'Subject-wise Performance'}
              </div>
                  ...
                  <div key={stat.subjectId}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-3 text-[10px]">
                        <span className="text-[var(--text-muted)]">{isBn ? 'গড়' : 'Avg'}: <span className="font-semibold text-[var(--text-primary)]">{stat.avg}/{stat.fullMarks}</span></span>
                        <span className="text-[var(--text-muted)]">{isBn ? 'পাস' : 'Pass'}: <span className="font-semibold" style={{ color: stat.passRate >= 60 ? 'var(--green)' : stat.passRate >= 40 ? 'var(--amber)' : 'var(--red)' }}>{stat.passRate}%</span></span>
                        <span className="text-[var(--text-muted)]">{isBn ? 'সর্বোচ্চ' : 'Highest'}: <span className="font-semibold text-[var(--green)]">{stat.highest}</span></span>
                      </div>
                    </div>
                    <div className="h-[6px] bg-[var(--border)] rounded-[3px]">
                      <div className="h-full rounded-[3px]" style={{ width: `${stat.passRate}%`, background: stat.passRate >= 60 ? 'var(--green)' : stat.passRate >= 40 ? 'var(--amber)' : 'var(--red)' }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeSubTab === 'position' && (
          <>
            {positionCheck.length > 0 ? (
              <div className={sectionCls}>
                <div className={sectionTitleCls}>
                  <AlertTriangle size={15} className="text-[var(--amber)]" />{isBn ? 'পজিশন ডুপ্লিকেশন' : 'Position Duplications'} ({positionCheck.length})
                </div>
                <p className="text-[11px] text-[var(--text-muted)] mb-3">{isBn ? 'একই মার্কস পাওয়া শিক্ষার্থীদের পজিশন চেক করুন' : 'Students with same marks sharing positions'}</p>
                <div className="space-y-2">
      ...
      {showExtraForm && (
        <div className="fixed inset-0 flex items-center justify-center p-4 z-[600] bg-black/50">
          <div className="bg-[var(--bg-primary)] rounded-[14px] max-w-[380px] w-full p-5 border border-[var(--border)]">
            <h3 className="text-[14px] font-semibold text-[var(--text-primary)] mb-3">{isBn ? 'এক্সট্রা মার্কস যোগ' : 'Add Extra Marks'}</h3>
            <div className="space-y-3">
              <div>
                <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn ? 'শিক্ষার্থী' : 'Student'}</label>
                <select value={extraForm.studentId} onChange={e => setExtraForm(p => ({ ...p, studentId: e.target.value }))} className={`${inputCls} w-full`}>
                  <option value="">{isBn ? 'নির্বাচন...' : 'Select...'}</option>
                  {classStudents.map(s => <option key={s.id} value={s.id}>{isBn ? s.nameBn || s.nameEn : s.nameEn}</option>)}
                </select>
              </div>
              <div>
                <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn ? 'ধরন' : 'Type'}</label>
                <select value={extraForm.type} onChange={e => setExtraForm(p => ({ ...p, type: e.target.value as 'attendance' | 'discipline' | 'homework' }))} className={`${inputCls} w-full`}>
                  {extraMarkTypes.map(t => <option key={t.key} value={t.key}>{t.label}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn ? 'প্রাপ্ত' : 'Marks'}</label>
                  <input type="number" value={extraForm.marks} onChange={e => setExtraForm(p => ({ ...p, marks: e.target.value }))} className={`${inputCls} w-full`} />
                </div>
                <div>
                  <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn ? 'সর্বোচ্চ' : 'Max'}</label>
                  <input type="number" value={extraForm.maxMarks} onChange={e => setExtraForm(p => ({ ...p, maxMarks: e.target.value }))} className={`${inputCls} w-full`} />
                </div>
              </div>
              <div>
                <label className="text-[11px] font-medium text-[var(--text-secondary)] mb-1 block">{isBn ? 'নোট' : 'Note'}</label>
                <input value={extraForm.note} onChange={e => setExtraForm(p => ({ ...p, note: e.target.value }))} className={`${inputCls} w-full`} />
              </div>
            </div>
            <div className="flex gap-2 justify-end mt-4">
              <button onClick={() => setShowExtraForm(false)} className="px-3.5 py-2 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[12px] cursor-pointer">{isBn ? 'বাতিল' : 'Cancel'}</button>
              <button onClick={handleSaveExtra} className={`${btnPrimary} text-[12px]`}>{isBn ? 'সংরক্ষণ' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}

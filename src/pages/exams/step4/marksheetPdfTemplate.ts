...
export async function generateMarksheetPDF(
      ...
      if (hasSub) {
        return subExams.map((se, si) => {
          ...
          return `<tr style="background:${bg};">
            ${si === 0 ? `<td rowspan="${subExams.length}" style="padding:${f.tdPadding};text-align:center;font-size:${f.tdFontSize};border-bottom:1px solid #94a3b8;color:#6b7280;vertical-align:middle;">${idx + 1}</td>` : ''}
            ${si === 0 ? `<td rowspan="${subExams.length}" style="padding:${f.tdPadding};font-size:${f.tdFontSize};font-weight:600;border-bottom:1px solid #94a3b8;color:#1e293b;vertical-align:middle;">${sm.subjectName || ''}</td>` : ''}
            <td style="padding:${f.tdSubPadding};text-align:center;font-size:${f.tdSubFontSize};border-bottom:1px solid #94a3b8;color:#6b7280;">${seName}</td>
            <td style="padding:${f.tdSubPadding};text-align:center;font-size:${f.tdSubFontSize};border-bottom:1px solid #94a3b8;color:#6b7280;">${se.fullMarks}</td>
            <td style="padding:${f.tdSubPadding};text-align:center;font-size:${f.tdFontSize};border-bottom:1px solid #94a3b8;font-weight:500;color:#1e293b;">${seObtained}</td>
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Times New Roman', serif; background: white; -webkit-print-color-adjust: exact; print-color-adjust: exact; }

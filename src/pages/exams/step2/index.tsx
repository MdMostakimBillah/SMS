...
export default function Step2Schedule() {
  ...
  const sortedRoutines = useMemo(() => [...filteredRoutines].sort((a, b) => a.date.localeCompare(b.date)), [filteredRoutines])
          ...
          <>
            <div className="flex justify-between items-center mb-3">
              <span className="text-[12px] text-[var(--text-secondary)]">
                {filteredRoutines.length} {isBn ? 'টি রুটিন' : 'routines'}
              </span>
              <button
                onClick={() => {
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex flex-col items-center shrink-0">
                        <div className="text-[10px] font-semibold text-[var(--text-muted)]">
                          {new Date(routine.date).toLocaleDateString(isBn ? 'bn-BD' : 'en-US', { weekday: 'short' })}
                        </div>
                        <div className="text-[16px] font-bold text-[var(--text-primary)]">{new Date(routine.date).getDate()}</div>
                        <div className="text-[9px] text-[var(--text-muted)]">
                          {new Date(routine.date).toLocaleDateString(isBn ? 'bn-BD' : 'en-US', { month: 'short' })}
                        </div>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-[11px] text-[var(--text-muted)] flex-wrap">
                          ...
                          <span>
                            {routine.classId} {routine.sectionId}
                          </span>
                        </div>
                      </div>
                        ...
                        <button
                          onClick={() =>
                            updateRoutine(routine.id, {
                              status:
                                routine.status === 'completed' ? 'upcoming' : routine.status === 'upcoming' ? 'in-progress' : 'completed',
                            })
                          }
          ...
          <>
            ...
            <div className={sectionCls}>
              <div className={sectionTitleCls}>
                ...
                {isBn ? 'বরাদ্দ আসন' : 'Assigned Seats'} ({filteredSeatPlans.length})
              </div>
                ...
                <table className="w-full text-xs min-w-[400px]">
                  <thead>
                    <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                      ...
                      <th className="py-2 px-2 text-left text-[10px] font-semibold text-[var(--text-muted)]">
                      ...
                      return (
                        <tr key={sp.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-secondary)]">
                          ...
                          <td className="py-2 px-2 text-[var(--text-secondary)]">
                            {sp.classId} {sp.sectionId}
                          </td>
                          <td className="py-2 px-2 text-center">
                            <button
                              onClick={() => removeSeatPlan(sp.id)}
                              className="w-6 h-6 rounded border border-[var(--border)] bg-[var(--bg-secondary)] flex items-center justify-center cursor-pointer text-[var(--text-muted)] hover:text-[var(--red)] mx-auto"
                            >
                              <Trash2 size={11} />
                            </button>
                          </td>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setRoutineForm((p) => ({ ...p, classId: e.target.value, sectionId: '' }))}
                    className={`${inputCls} w-full`}
                  >
                    <option value="">{isBn ? 'নির্বাচন...' : 'Select...'}</option>
                    {classOptions.map((c) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setRoutineForm((p) => ({ ...p, sectionId: e.target.value }))}
                    className={`${inputCls} w-full`}
                  >
                    <option value="">{isBn ? 'নির্বাচন...' : 'Select...'}</option>

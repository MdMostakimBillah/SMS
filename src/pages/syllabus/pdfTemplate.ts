      ...
      topicRows += `
        <tr>
          ...
          <td style="padding:5px 6px;border:1px solid #e5e7eb;">
            <div style="font-size:10px;font-weight:600;color:#111827;">${escapeHtml(isBn ? t.titleBn : t.title)}</div>
            ${options.showTopicDesc ? `<div style="font-size:8px;color:#6b7280;margin-top:1px;">${escapeHtml(isBn ? t.descriptionBn : t.description)}</div>` : ''}
          </td>
          <td style="padding:5px 6px;border:1px solid #e5e7eb;text-align:center;font-size:10px;color:#374151;">${t.weekNo || '-'}</td>
          <td style="padding:5px 6px;border:1px solid #e5e7eb;text-align:center;font-size:10px;color:#374151;">${escapeHtml(t.startDate || '-')}<br/>${escapeHtml(t.endDate || '-')}</td>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Times New Roman', serif; color: #1f2937; font-size: 11px; padding: 8mm; }
...
<div style="display:flex;align-items:center;justify-content:space-between;padding-bottom:7px;border-bottom:2px solid ${brandColor};margin-bottom:7px">
  <div style="display:flex;align-items:center;gap:10px">
    ...
    <div>
      <div style="font-size:16px;font-weight:800;color:${brandColor};letter-spacing:0.3px;font-family:'Times New Roman',serif;">${escapeHtml(b.schoolNameBn && isBn ? b.schoolNameBn : b.schoolName)}</div>
  ...
  <div style="text-align:right;font-size:8px;color:#666;line-height:1.7">
    ...
    <div style="font-size:9px;color:#6b7280;line-height:1.8">
      <strong>${isBn ? 'শ্রেণি' : 'Class'}:</strong> ${escapeHtml(syllabus.className)} &nbsp;|&nbsp;
      <strong>${isBn ? 'বিষয়' : 'Subject'}:</strong> ${escapeHtml(syllabus.subjectName)} &nbsp;|&nbsp;
      <strong>${isBn ? 'সেশন' : 'Session'}:</strong> ${escapeHtml(syllabus.sessionId)}
    </div>
  </div>
</div>
...
${syllabus.chapters.map((ch, ci) => {
  const chDesc = options.showChapterDesc ? `<div style="font-size:9px;color:#6b7280;margin-bottom:4px;">${escapeHtml(isBn ? ch.descriptionBn : ch.description)}</div>` : ''
  return `
  <div style="margin-bottom:10px;">
    <div style="background:${brandColor}10;border-left:3px solid ${brandColor};padding:5px 8px;margin-bottom:4px;border-radius:0 4px 4px 0;">
      ...
      <span style="font-size:11px;font-weight:600;color:#111827;margin-left:4px;">${escapeHtml(isBn ? ch.titleBn : ch.title)}</span>
    </div>
    ${chDesc}

...
export default function HRPage() {
  ...
  const [incForm, setIncForm] = useState({
    teacherId: '',
    type: 'annual' as 'annual' | 'performance' | 'special',
    percentage: '',
    reason: '',
  })
  const [bonForm, setBonForm] = useState({
    teacherId: '',
    type: 'festival' as 'festival' | 'performance' | 'attendance' | 'special',
    amount: '',
    reason: '',
    month: '',
  })
  ...
  const paginatedIncrements = useMemo(
    () => filteredIncrements.slice((page - 1) * perPage, page * perPage),
    [filteredIncrements, page, perPage]
  )
  ...
  const paginatedAssignments = useMemo(
    () => filteredAssignments.slice((page - 1) * perPage, page * perPage),
            ...
            <div className="flex flex-col gap-1.5">
              {teacherCompositeScores.slice(0, 10).map((t, i) => (
                <div key={t.id} className="py-[10px] px-3 rounded-lg bg-[var(--bg-secondary)] transition-all">
                  <div className="flex items-center gap-[10px]">
                    <span className={`text-[13px] font-bold w-[24px] ${i < 3 ? 'text-[var(--amber)]' : 'text-[var(--text-muted)]'}`}>
                      #{i + 1}
                    </span>
                    <div
                      className="w-8 h-8 rounded-lg flex items-center justify-center text-[9px] font-semibold text-white shrink-0"
                      style={{ background: getAvatarGradient(t.id) }}
                    >
                      {getInitials(t.nameEn)}
                    </div>
                  ...
                  <div className="grid grid-cols-4 gap-1.5 mt-2">
                    {[
                      ...
                      <div key={d.label} className="text-center p-1 rounded-md bg-[var(--bg-primary)]">
                        ...
                        <div className="text-[13px] font-semibold" style={{ color: d.color }}>
                          {d.val}
                        </div>
                      </div>
                    ))}
                        ...
                        <div className="flex items-center gap-[10px] flex-wrap">
                          ...
                          <div className="flex-1 min-w-[0]">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              <span
                                className="text-[11px] py-[2px] px-2 rounded-full font-medium"
                    ...
                    return (
                      <div key={rec.id} className="py-2 px-3 rounded-lg bg-[var(--bg-secondary)] flex items-center gap-[10px]">
                        ...
                        <div className="flex-1">
                          ...
                          <span
                            className="text-[11px] py-px px-2 rounded-xl font-medium ml-2"
                            style={{ background: bg[rec.type], color: colors[rec.type] }}
                          >
                            {labels[rec.type]}
                          </span>
                        ...
                        <span
                          className={`text-[11px] py-[3px] px-2 rounded-full font-medium ${rec.status === 'approved' ? 'bg-[var(--green-light)]' : 'bg-[var(--red-light)]'} ${rec.status === 'approved' ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
                        >
            ...
            <div className="flex h-[32px] shrink-0">
              <button
                onClick={() => {
                  ...
                  setIncDateFrom(d.toISOString().split('T')[0])
                  setIncDateTo(new Date().toISOString().split('T')[0])
                }}
              ...
              <thead>
                <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                  <th className="py-[10px] px-2 w-9">
                    <input
                      type="checkbox"
                      checked={selectedInc.length === filteredIncrements.length && filteredIncrements.length > 0}
                      onChange={toggleAllInc}
              ...
              <tbody>
                {paginatedIncrements.map((inc) => {
                  ...
                  return (
                    <tr
                      key={inc.id}
                      className={`border-b border-[var(--border)] ${selectedInc.includes(inc.id) ? 'bg-[rgba(99,102,241,0.04)]' : 'bg-transparent'}`}
                      onMouseEnter={(e) => {
                        if (!selectedInc.includes(inc.id)) e.currentTarget.style.background = 'var(--bg-secondary)'
                      }}
                      onMouseLeave={(e) => {
                        if (!selectedInc.includes(inc.id)) e.currentTarget.style.background = 'transparent'
                      }}
                    >
                      <td className="py-2 px-2">
                        <input
                          type="checkbox"
                          checked={selectedInc.includes(inc.id)}
            ...
            <div className="py-[10px] px-0 flex justify-between items-center border-t border-[var(--border)] mt-2 flex-wrap gap-2">
              ...
              <div className="flex gap-[3px] items-center">
                ...
                {[
                  [
                    <ChevronRight size={12} />,
                    () => setPage((p) => Math.min(incrementTotalPages, p + 1)),
                    page === incrementTotalPages,
                  ] as [React.ReactNode, () => void, boolean],
                  [<ChevronsRight size={12} />, () => setPage(incrementTotalPages), page === incrementTotalPages] as [
                    React.ReactNode,
                    () => void,
                    boolean,
                  ],
            ...
            <table className="w-full text-xs min-w-[640px]">
              <thead>
                <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                  <th className="py-[10px] px-2 w-9">
                    <input
                      type="checkbox"
                      checked={selectedBon.length === filteredBonuses.length && filteredBonuses.length > 0}
                      onChange={toggleAllBon}
                      className="w-[13px] h-[13px] cursor-pointer accent-[var(--brand)]"
                    />
                  </th>
                  {[
              ...
              <tbody>
                {paginatedBonuses.map((bon) => (
                  <tr
                    key={bon.id}
                    className={`border-b border-[var(--border)] ${selectedBon.includes(bon.id) ? 'bg-[rgba(99,102,241,0.04)]' : 'bg-transparent'}`}
                    onMouseEnter={(e) => {
                      if (!selectedBon.includes(bon.id)) e.currentTarget.style.background = 'var(--bg-secondary)'
                    }}
                    onMouseLeave={(e) => {
                      if (!selectedBon.includes(bon.id)) e.currentTarget.style.background = 'transparent'
                    }}
                  >
                    <td className="py-2 px-2">
                      <input
                        type="checkbox"
                        checked={selectedBon.includes(bon.id)}
                        onChange={() => toggleBon(bon.id)}
                        className="w-[13px] h-[13px] cursor-pointer accent-[var(--brand)]"
                      />
                    </td>
                    ...
                    <td className="py-2 px-2">
                      <span
                        className={`text-[10px] py-[2px] px-[6px] rounded-[5px] font-medium ${bon.type === 'festival' ? 'bg-[var(--amber-light)]' : bon.type === 'performance' ? 'bg-[var(--brand-light)]' : bon.type === 'attendance' ? 'bg-[var(--green-light)]' : 'bg-[var(--teal-light)]'} ${bon.type === 'festival' ? 'text-[var(--amber)]' : bon.type === 'performance' ? 'text-[var(--brand)]' : bon.type === 'attendance' ? 'text-[var(--green)]' : 'text-[var(--teal)]'}`}
                      >
                        {bon.type}
                      </span>
                    </td>
          ...
          {filteredBonuses.length > perPage && (
            <div className="py-[10px] px-0 flex justify-between items-center border-t border-[var(--border)] mt-2 flex-wrap gap-2">
              ...
              <div className="flex gap-[3px] items-center">
                ...
                {[
                  [<ChevronRight size={12} />, () => setPage((p) => Math.min(bonusTotalPages, p + 1)), page === bonusTotalPages] as [
                    React.ReactNode,
                    () => void,
                    boolean,
                  ],
                  [<ChevronsRight size={12} />, () => setPage(bonusTotalPages), page === bonusTotalPages] as [
                    React.ReactNode,
                    () => void,
                    boolean,
                  ],
            ...
            <div className="flex h-[32px] shrink-0">
              <button
                onClick={() => {
                  const d = new Date()
                  d.setMonth(d.getMonth() - 6)
                  setProDateFrom(d.toISOString().split('T')[0])
                  setProDateTo(new Date().toISOString().split('T')[0])
                }}
                className="px-[10px] border border-[var(--border)] border-r-0 cursor-pointer font-[inherit] text-[11px] bg-[var(--bg-secondary)] text-[var(--text-secondary)] hover:bg-[var(--brand-light)] hover:text-[var(--brand)] transition-all rounded-l-lg"
              >
                6{isBn ? 'মাস' : 'M'}
                ...
                <table className="w-full text-xs min-w-[600px]">
                  <thead>
                    <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                      <th className="py-[10px] px-2 w-9">
                        <input
                          type="checkbox"
                          checked={selectedPro.length === filteredPromotions.length && filteredPromotions.length > 0}
                          onChange={toggleAllPro}
                          className="w-[13px] h-[13px] cursor-pointer accent-[var(--brand)]"
                        />
                      </th>
                      {[
                  ...
                  <tbody>
                    {paginatedPromotions.map((p) => (
                      <tr
                        key={p.id}
                        className={`border-b border-[var(--border)] ${selectedPro.includes(p.id) ? 'bg-[rgba(99,102,241,0.04)]' : 'bg-transparent'}`}
                        onMouseEnter={(e) => {
                          if (!selectedPro.includes(p.id)) e.currentTarget.style.background = 'var(--bg-secondary)'
                        }}
                        onMouseLeave={(e) => {
                          if (!selectedPro.includes(p.id)) e.currentTarget.style.background = 'transparent'
                        }}
                      >
                        <td className="py-2 px-2">
                          <input
                            type="checkbox"
                            checked={selectedPro.includes(p.id)}
                ...
                <div className="py-[10px] px-0 flex justify-between items-center border-t border-[var(--border)] mt-2 flex-wrap gap-2">
                  ...
                  <div className="flex gap-[3px] items-center">
                    ...
                    {[
                      [
                        <ChevronRight size={12} />,
                        () => setPage((p) => Math.min(promotionTotalPages, p + 1)),
                        page === promotionTotalPages,
                      ] as [React.ReactNode, () => void, boolean],
                      [<ChevronsRight size={12} />, () => setPage(promotionTotalPages), page === promotionTotalPages] as [
                        React.ReactNode,
                        () => void,
                        boolean,
                      ],
          ...
          {activeTeachers.length > perPage && (
            <div className="py-[10px] px-0 flex justify-between items-center border-t border-[var(--border)] mt-2 flex-wrap gap-2">
              ...
              <div className="flex gap-[3px] items-center">
                ...
                {[
                  [<ChevronRight size={12} />, () => setPage((p) => Math.min(salaryTotalPages, p + 1)), page === salaryTotalPages] as [
                    React.ReactNode,
                    () => void,
                    boolean,
                  ],
                  [<ChevronsRight size={12} />, () => setPage(salaryTotalPages), page === salaryTotalPages] as [
                    React.ReactNode,
                    () => void,
                    boolean,
                  ],
                    ...
                    <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                      <th className="p-2 w-9">
                        <input
                          ...
                          onChange={() =>
                            setSelectedAssign((p) =>
                              p.length === filteredAssignments.length ? [] : filteredAssignments.map((tf) => tf.id)
                            )
                          }
                          className="w-[13px] h-[13px] cursor-pointer accent-[var(--brand)]"
                        />
              ...
              <div className="py-[10px] px-0 flex justify-between items-center border-t border-[var(--border)] mt-2 flex-wrap gap-2">
                ...
                <div className="flex gap-[3px] items-center">
                  ...
                  {[
                    [
                      <ChevronRight size={12} />,
                      () => setPage((p) => Math.min(assignmentTotalPages, p + 1)),
                      page === assignmentTotalPages,
                    ] as [React.ReactNode, () => void, boolean],
                    [<ChevronsRight size={12} />, () => setPage(assignmentTotalPages), page === assignmentTotalPages] as [
                      React.ReactNode,
                      () => void,
                      boolean,
                    ],
        ...
        <div className={modalOverlayCls} onClick={() => setFacDeleteConfirm(null)}>
          <div className={`modal-content ${modalStyleCls} max-w-[380px]`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setFacDeleteConfirm(null)}
      ...
      {assignDeleteConfirm && (
        <div className={modalOverlayCls} onClick={() => setAssignDeleteConfirm(null)}>
          <div className={`modal-content ${modalStyleCls} max-w-[380px]`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setAssignDeleteConfirm(null)}
                className="py-2 px-[14px] rounded-lg bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-secondary)] text-[13px] cursor-pointer font-[inherit]"
              >
                {isBn ? 'বাতিল' : 'Cancel'}
              </button>

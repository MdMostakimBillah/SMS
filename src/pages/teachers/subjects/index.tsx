            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              ...
              <div>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', padding:'8px', background:'var(--bg-secondary)', border:'1px solid var(--border)', borderRadius:'8px', minHeight:'40px' }}>
                  {departments.map(d => (
                    <label key={d.id} style={{ display:'flex', alignItems:'flex-start', gap:'5px', cursor:'pointer', padding:'4px 8px', borderRadius:'6px', background: newDeptIds.includes(d.id) ? 'var(--brand-light)' : 'var(--bg-primary)', border:`1px solid ${newDeptIds.includes(d.id) ? 'var(--brand)' : 'var(--border)'}`, fontSize:'12px', color: newDeptIds.includes(d.id) ? 'var(--brand)' : 'var(--text-secondary)', transition:'all 0.15s' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', gap:'8px' }}>
          ...
          {fDept && (
            <button onClick={() => setFDept('')}
              style={{ display:'flex', alignItems:'flex-start', gap:'4px', padding:'4px 10px', borderRadius:'6px',
                ...
                : filtered.map((s, i) => (
                  <tr key={s.id}
                    ...
                    <td style={{ padding:'10px 12px' }}>
                      <div style={{ display:'flex', alignItems:'flex-start', gap:'8px' }}>
                        <div style={{ width:'32px', height:'32px', borderRadius:'8px', background:'var(--green-light)', display:'flex', alignItems:'flex-start', justifyContent:'flex-start', flexShrink:0 }}>

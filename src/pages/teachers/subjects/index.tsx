...
export default function SubjectsPage() {
  ...
  const filtered = useMemo(() =>
    fDept ? subjects.filter(s => s.departmentId === fDept) : subjects
  , [subjects, fDept])
  ...
  const handleAdd = () => {
    ...
    addSubject({
      ...
      name: newName.trim(), nameBn: newNameBn.trim(), departmentId: newDeptId,
      createdAt: now, updatedAt: now,
    })
    setNewName(''); setNewNameBn(''); setNewDeptId(''); setShowAdd(false)
  }

  const handleEdit = () => {
    if (!editS || !newName.trim()) return
    updateSubject(editS.id, { name: newName.trim(), nameBn: newNameBn.trim(), departmentId: newDeptId })
    setEditS(null); setNewName(''); setNewNameBn(''); setNewDeptId('')
  }

  const startEdit = (s: Subject) => {
    setNewName(s.name); setNewNameBn(s.nameBn); setNewDeptId(s.departmentId); setEditS(s)
  }
  ...
  return (
    <div>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:600, display:'flex', alignItems:'center', justifyContent:'center', padding:'16px' }}>
          <div style={{ background:'var(--bg-primary)', borderRadius:'14px', maxWidth:'400px', width:'100%', padding:'20px', border:'1px solid var(--border)' }}>
            ...
            <div style={{ display:'flex', gap:'8px', justifyContent:'flex-end', marginTop:'16px' }}>
              <button onClick={() => { setShowAdd(false); setEditS(null); setNewName(''); setNewNameBn(''); setNewDeptId('') }}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'16px', flexWrap:'wrap' }}>
        ...
        <button onClick={() => { setShowAdd(true); setNewName(''); setNewNameBn(''); setNewDeptId('') }}

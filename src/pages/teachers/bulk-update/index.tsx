...
export default function TeacherBulkUpdatePage() {
  ...
  const inpCls = "py-[7px] px-[9px] rounded-[8px] border border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-secondary)] text-[12px] font-[inherit] cursor-pointer outline-none"
  ...
  return (
    <div>
      ...
      <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-[240px_1fr]'} gap-[10px] mb-[10px]`}>
        ...
        <div className="bg-[var(--bg-primary)] border border-[color:var(--op-color)] rounded-[12px] py-[12px] px-[14px]"
          style={{ '--op-color': opInfo.color } as React.CSSProperties}>
          <div className="text-[11px] font-semibold uppercase tracking-[0.5px] mb-[10px]"
          ...
          {op === 'photo' ? (
            <div className="py-[10px] px-[10px] rounded-[8px] text-[13px]"
        ...
        <div className="py-[10px] px-[14px] border-b border-[var(--border)] flex items-center justify-between bg-[var(--bg-secondary)]">
          <div className="flex items-center gap-[8px]">
            ...
            <span className="text-[13px] font-medium text-[var(--text-primary)]">
              ...
              {selected.length > 0 && <span className="ml-[8px]" style={{ color: opInfo.color } as React.CSSProperties}>· {selected.length} {isBn ? 'নির্বাচিত' : 'selected'}</span>}
            </span>
          </div>
          {readyCount > 0 && (
            <span className="text-[12px] py-[3px] px-[10px] rounded-[6px] font-medium"
          ...
          <table className="w-full border-collapse text-[12px]">
            <thead>
              <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border)]">
                ...
                <th className="py-[10px] px-[8px] text-left text-[10px] font-semibold text-[var(--text-muted)] uppercase min-w-[50px]">{isBn ? 'ছবি' : 'Photo'}</th>
                <th className="py-[10px] px-[8px] text-left text-[10px] font-semibold text-[var(--text-muted)] uppercase min-w-[165px]">{isBn ? 'নাম / আইডি' : 'Name / ID'}</th>
                <th className="py-[10px] px-[8px] text-left text-[10px] font-semibold text-[var(--text-muted)] uppercase min-w-[80px]">{isBn ? 'বর্তমান মান' : 'Current'}</th>
                <th className="py-[10px] px-[8px] text-left text-[10px] font-semibold uppercase min-w-[200px]"
            ...
            <tbody>
              {filtered.length === 0
                ? <tr><td colSpan={6} className="py-[30px] text-center text-[var(--text-muted)]">
                  ...
                  <tr key={t.id}
                    ...
                    <td className="py-[7px] px-[8px]">
                      <div className="w-[32px] h-[38px] rounded-[6px] overflow-hidden bg-[var(--bg-secondary)] border border-[var(--border)] flex items-center justify-center">
                        {t.photo ? <img src={t.photo} alt="" className="w-full h-full object-cover" /> : <span className="text-[10px] text-[var(--text-muted)]">{t.nameEn.split(' ').map(n => n[0]).slice(0, 2).join('')}</span>}
                      </div>
                    </td>
                    ...
                    <td className="py-[6px] px-[8px]"
                      ...
                      {op === 'photo' ? (
                        <label className={`flex items-center gap-[5px] py-[6px] px-[10px] rounded-[7px] border text-[11px] cursor-pointer font-medium w-fit ${photoMap[t.id] ? 'border-[var(--green)] text-[var(--green)]' : ''}`}
                          style={{ background: photoMap[t.id] ? 'var(--green-light)' : opInfo.bg, borderColor: photoMap[t.id] ? 'var(--green)' : opInfo.color, color: photoMap[t.id] ? 'var(--green)' : opInfo.color } as React.CSSProperties}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-[10px]">
        ...
        <div className="flex gap-[8px]">
          ...
          <button onClick={applyChanges} disabled={readyCount === 0}
            ...
            style={{ background: readyCount === 0 ? 'var(--border-2)' : opInfo.color, boxShadow: readyCount > 0 ? `0 4px 14px ${opInfo.color}50` : 'none' } as React.CSSProperties}>
